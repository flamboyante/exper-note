...
![图 12-3-11 往返布线延迟对 sclk\_out/rxd 采样的影响](../../assets/trm-12.3-spi/p0784-img01.png)
...
![图 12-3-12 最大 sclk\_out/ssi\_clk 比率](../../assets/trm-12.3-spi/p0785-img01.png)
...
| ------------- | --------------------- | ---------------------- | ------------------------------------------------------------------- | -------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | ----: | ----------------------------------- |
| `0x000`       | `CTRLR0`              | 控制寄存器 0                | 返回保存值与 implemented\_mask 的交集；保留位和未实现位返回 0。                          | 按 writable\_mask 合并保存；配置 DFS/TMOD/SPI\_FRF 等，影响后续事务，不单独启动传输。                     | 可写掩码 `0x01cf7f1f`；`DFS/TMOD/SPI_FRF/SCPOL/SCPH` 进入事务行为，`CFS/SLV_OE/SRL/SSTE/SPI_HYPERBUS_EN` 保存读回；HyperBus 位当前不产生事务副作用 | `A/B` | `T12-R`、`RT/UB-ACT/LX-ACT/HW`       |
...
| `0x008`       | `SSIENR`              | SSI 使能寄存器              | 返回当前 SSIC\_EN 状态。                                                   | 写 0 立即停止传输并清空 TX/RX FIFO；写 1 使能控制器，在片选和数据条件满足时启动。                                | 仅 bit 0；清零停止传输并清空 TX/RX FIFO                                                                                           |   `A` | `T12-F/T12-R`                       |
...
| `0x014`       | `BAUDR`               | 波特率选择寄存器               | 返回当前偶数分频值；bit 0 和高 16 位返回 0。                                        | 清除 bit 0 后保存偶数分频值；更新后续 sclk\_out 频率，写 0 禁止串行时钟。                                  | 可写 `0x0000fffe`；bit 0 固定为 0，值 0 禁止输出串行时钟                                                                               |   `A` | `T12-F/T12-R`                       |
...
| `0x068`       | `SSI_CTRL`            | TRM 混入的 SoC 控制寄存器      | 在 0x91585068 返回 HI\_SYS\_CONFIG 控制位与三个实例状态；SPI base+0x068 按 DR2 读取。 | 仅在 0x91585068 更新可写包装位，如 spi0\_xip\_en 与 RXDS 配置；SPI base+0x068 按 DR2 写入 TX FIFO。 | 不属于 SPI `base+0x068`；实际地址 `0x91585068`，SPI 此偏移是 `DR2`                                                                  |   `E` | `MAP`、`UB-ACT`、`RT`                 |
...
| `0x0fc`       | `XIP_MODE_BITS`       | 5.3 XIP mode bits      | FMC/XIP profile 返回低 16 位；QSPI profile 返回 0。                                  | FMC 只保存低 16 位并用于 XIP mode-bits 阶段；QSPI 写入忽略。                                      | 12.3 QSPI 表未定义该偏移；仅 `spi0` FMC/XIP profile 实现                                                                              | `A/D` | `T53-R`；`T12-R` 无此项               |
| `0x100`       | `XIP_INCR_INST`       | 5.3 XIP INCR 指令        | FMC/XIP profile 返回低 16 位；QSPI profile 返回 0。                                  | FMC 只保存低 16 位并用于 INCR XIP；QSPI 写入忽略。                                               | 12.3 QSPI 表未定义该偏移；仅 `spi0` FMC/XIP profile 实现                                                                              | `A/D` | `T53-R`；`T12-R` 无此项               |
| `0x104`       | `XIP_WRAP_INST`       | 5.3 XIP WRAP 指令        | FMC/XIP profile 返回低 16 位；QSPI profile 返回 0。                                  | FMC 只保存低 16 位并用于 WRAP XIP；QSPI 写入忽略。                                               | 12.3 QSPI 表未定义该偏移；仅 `spi0` FMC/XIP profile 实现                                                                              | `A/D` | `T53-R`；`T12-R` 无此项               |
...
| `0x120`       | `SPIDR`               | 内部 DMA SPI 指令寄存器       | 返回保存的 SPI\_INST\[15:0]。                                             | 只保存 SPI\_INST\[15:0]；后续内部 DMA SPI 事务把它作为指令阶段内容。                                  | 低 16 位可写并参与 IDMA 事务                                                                                                    |   `A` | `T12-R/T53-R`、`RT/UB-ACT/LX-ACT`    |
...
| SoC 包装寄存器 | `SSI_CTRL`                          | 由 HI\_SYS\_CONFIG 返回控制位和状态位 | 只更新包装层可写位               |
...
| 2     | 随配置变化 | `IDMAE`       | `0x0` | Internal DMA Enable。仅应在 `CTRLR0.FRF=0` 且 `CTRLR0.SPI_FRF>0` 时使能。                                       |    `A` | K230 选择 SSIC\_HAS\_DMA=2，置位后进入内部 AXI DMA 路径。             | `T12-R/RT/UB-ACT/LX-ACT` |
...
| 17:14 | R/W | `rxds_delay_num`      | `0x1` | RXDS 采样延迟数量。                       |    `E` | U-Boot 结构体明确为 4 位；保存和读回，由 HI\_SYS\_CONFIG 包装层提供。           | `T12-R/UB-ACT/MAP` |
...
> **K230 依据与 QEMU 读写实现：**
>
> - **软件与集成证据：** U-Boot `ssi_ctrl_t` 给出无重叠的完整 32 位布局，并把绝对地址定义为 `0x91585000+0x68`。SPI 本体同偏移是 `DR2`，因此该寄存器必须属于独立 HI\_SYS\_CONFIG 设备。
...
> - **读取实现：** 由 HI\_SYS\_CONFIG 组合保存的 bits\[17:13]/bit0 与三个 SPI 实例的动态 mode/sleep：mode 取各实例 `CTRLR0.SPI_FRF[1:0]`，sleep 返回包装层锁存的延迟状态；SPI 控制器 MMIO `base+0x068` 仍执行 DR2 FIFO 访问。
...
| 27:26 | 随配置变化 | `XIP_MBL`               | `0x1` | XIP mode bits 长度，仅在 `XIP_MD_BIT_EN=1` 时有效：`0=2 bits`，`1=4 bits`，`2=8 bits`，`3=16 bits`。                                                          |  `A/B` | spi0 XIP 使用；QSPI reset 可包含该编码，但普通 enhanced/IDMA 不消费它。                 | `T12-R/T53-R`                     |
...
| 20    | 随配置变化 | `XIP_INST_EN`           | `0x0` | XIP 指令阶段使能。置 1 后 XIP 传输包含指令阶段，操作码根据 AHB 传输类型从 `XIP_INCR_INST` 或 `XIP_WRAP_INST` 选择。                                                              |  `A/B` | spi0 决定是否从 XIP\_INCR\_INST/XIP\_WRAP\_INST 发送指令。                      | `T12-R/T53-R/RT/UB-ACT/LX-ACT`    |
...
| 7     | 随配置变化 | `XIP_MD_BIT_EN`         | `0x0` | XIP mode bits 使能。置 1 后，在 XIP 地址阶段之后插入 `XIP_MODE_BITS` 中的 mode bits。                                                                               |  `A/B` | 只影响 spi0 FMC 的 XIP transaction；普通 enhanced/IDMA 忽略该位。                    | `T12-R/T53-R`                     |
...
> - **`bits[15:11] WAIT_CYCLES`、`bit[10]`、`bits[9:8] INST_L`、`bit[7] XIP_MD_BIT_EN`、`bit[6]`、`bits[5:2] ADDR_L`、`bits[1:0] TRANS_TYPE`：** bits10/6 固定读 0。普通 enhanced/IDMA 由 `INST_L/ADDR_L/TRANS_TYPE/WAIT_CYCLES` 组织“指令 → 地址 → 等待 → 数据”；`XIP_MD_BIT_EN` 只在 XIP transaction 中插入 mode bits。`TRANS_TYPE=3` 为保留编码，不得启动事务。
...
| 15:0  | R/W | `XIP_MD_BITS`      | `0x0` | XIP 地址阶段之后发送的 mode bits。 |  `A/D` | `spi0` 由 `SPI_CTRLR0.XIP_MD_BIT_EN/XIP_MBL` 使用；QSPI profile RAZ/WI。 | `T53-R`；`T12-R` 无此项 |

> \[!NOTE]
> **K230 依据与 QEMU 读写实现：**
>
> - **TRM/SDK 依据（`bits[31:16]`、`bits[15:0] XIP_MD_BITS`）：** 5.3 FMC 规定高位读 0、低 16 位保存；12.3 QSPI 寄存器表没有 `0x0fc`。RT 的共同结构体只用于覆盖最大寄存器布局，不能证明三个实例都实现该地址。
> - **读取实现：** FMC/XIP profile 返回保存值 `& 0xffff`；QSPI profile 返回 0。
> - **写入实现：** FMC/XIP profile 按 `writable_mask=0xffff` 更新；QSPI profile 写入忽略。
> - **副作用与依赖：** spi0 且 `SPI_CTRLR0.XIP_MD_BIT_EN=1` 时，按 `XIP_MBL` 从低位取 mode bits 插入 XIP 地址与数据阶段之间；普通 enhanced/IDMA 不读取该寄存器。
...
- 偏移：`0x100`
- 总复位值：`0x0`
- 来源：TRM 5.3 FMC；SDK 结构体确认最大布局偏移，不证明 QSPI 实现该地址。

| 位     | 访问  | 名称               | 复位值   | 中文说明                                                     | K230等级 | K230实现注释                             | 依据         |
| ----- | --- | ---------------- | ----- | -------------------------------------------------------- | -----: | ------------------------------------ | ---------- |
| 31:16 | R   | `RSVD_INCR_INST` | `0x0` | 保留位，读取为 0。                                               |    `D` | 保留位读零、写忽略。                           | `T53-R`    |
| 15:0  | R/W | `INCR_INST`      | `0x0` | AHB INCR 类型 XIP 访问使用的指令码；实际指令长度由 `SPI_CTRLR0.INST_L` 决定。 |  `A/D` | `spi0` XIP 指令路径使用；QSPI profile RAZ/WI。 | `T53-R`；`T12-R` 无此项 |

> \[!NOTE]
> **K230 依据与 QEMU 读写实现：**
>
> - **TRM/SDK 依据（`bits[31:16]`、`bits[15:0] INCR_INST`）：** 5.3 FMC 规定高位固定读 0、低 16 位保存 XIP INCR opcode；12.3 QSPI 表没有该偏移。
> - **读取实现：** FMC/XIP profile 返回保存值 `& 0xffff`；QSPI profile 返回 0。
> - **写入实现：** FMC/XIP profile 按 `writable_mask=0xffff` 更新；QSPI profile 写入忽略。
> - **副作用与依赖：** spi0 XIP INCR 请求且 `XIP_INST_EN=1` 时按 `INST_L` 发送该指令；普通 enhanced/IDMA 不使用该寄存器。
...
| 15:0  | R/W | `WRAP_INST`      | `0x0` | AHB WRAP 类型 XIP 访问使用的指令码；实际指令长度由 `SPI_CTRLR0.INST_L` 决定。 |  `A/D` | `spi0` XIP 指令路径使用；QSPI profile RAZ/WI。 | `T53-R`；`T12-R` 无此项 |
...
> - **读取实现：** FMC/XIP profile 返回保存值 `& 0xffff`；QSPI profile 返回 0。
> - **写入实现：** FMC/XIP profile 按 `writable_mask=0xffff` 更新；QSPI profile 写入忽略。
> - **副作用与依赖：** spi0 XIP WRAP 请求且指令阶段启用时按 `INST_L` 使用；普通 enhanced/IDMA 不使用该寄存器。
...
| 冲突                                    | 证据                                                                            | 当前裁决                                                                             |
| ------------------------------------- | ----------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| `SSI_CTRL @ 0x068` 与 `DR2 @ 0x068` 重叠 | U-Boot 定义 `SSI_CTRL = 0x91585000 + 0x68`；SDK 将 SPI `0x060–0x0ec` 映射为 `dr[36]` | SPI 控制器 `base + 0x068` 是 `DR2`；`SSI_CTRL` 的实际地址是 `0x91585068`，属于 HI\_SYS\_CONFIG |
...
`SSI_CTRL` 不属于这两个 profile 的控制器内部寄存器数组，而应由独立的 HI\_SYS\_CONFIG 模型在 `0x91585068` 提供。内部 DMA、256 深 FIFO 和九路独立中断则是当前 K230 SDK 的共同要求。
...
SDK 公共结构体还定义了 `XIP_MODE_BITS`、`XIP_INCR_INST`、`XIP_WRAP_INST` 等成员，用于覆盖最大寄存器布局和维持后续寄存器偏移。12.3 QSPI 寄存器表没有这三个偏移，三套 SDK 的普通 enhanced/IDMA 路径也没有写它们，因此不能据此认定 QSPI 实例实现这些寄存器。5.3 FMC 给出了完整 XIP 能力集合，但当前 RT-Smart 配置没有启用 concurrent XIP 和 XIP write 扩展，因此 `0x108–0x11c` 与 `0x140–0x148` 仍不能仅凭 5.3 的通用能力表认定为当前 profile 的有效功能寄存器。
...
| `MAP`    | K230 地址图与 SDK `board.h`                                                                  | OPI、HI\_SYS\_CONFIG 和 XIP window 的物理地址归属          |
...
| `XIP_MBL[27:26]`               |  `A/B` | XIP mode bits 长度，参与 XIP 命令生成                  | reset 编码可见，但普通 enhanced/IDMA 不消费，不产生 XIP 行为       | `T12-R/T53-R`                       |
...
| `XIP_MD_BIT_EN` bit 7          |  `A/B` | XIP transaction 置位后插入 `XIP_MODE_BITS`         | 普通 enhanced/IDMA 忽略该位，不读取 `XIP_MODE_BITS`           | `T53-F/T12-R`                       |
...
|       `0x0fc` | `XIP_MODE_BITS`    | `A/D` | `spi0` FMC/XIP profile 低 16 位有效；QSPI profile RAZ/WI | `T53-R`；`T12-R` 无此项                 |
|       `0x100` | `XIP_INCR_INST`    | `A/D` | `spi0` FMC/XIP profile 低 16 位有效；QSPI profile RAZ/WI | `T53-R`；`T12-R` 无此项                 |
|       `0x104` | `XIP_WRAP_INST`    | `A/D` | `spi0` FMC/XIP profile 低 16 位有效；QSPI profile RAZ/WI | `T53-R`；`T12-R` 无此项                 |
...
| `XIP_MODE_BITS/XIP_INCR_INST/XIP_WRAP_INST`     | 仅 FMC/XIP profile 保存低 16 位 | 12.3 QSPI 表未定义三个偏移；SDK 公共结构体不能证明实例存在性       |
...
| 6:5   | `ssi0_spi_mode`      | RO  | 反映 `spi0` 当前 SPI\_FRF/profile 状态 |
...
| `SSI_CTRL`        |                          `0x00004000` | 独立 HI\_SYS\_CONFIG 寄存器        | `T12-R/T53-R`、`UB-ACT` |
...
- `SSI_CTRL`：位于独立 HI\_SYS\_CONFIG 设备。
...
### QEMU 实施、Patch 与 qtest 入口

本章只维护 TRM/SDK 证据、冲突裁决和目标寄存器契约，不再维护会随分支变化的
Patch 顺序、实现状态或 qtest 结果。

统一实施入口：

- [K230 SPI/QSPI 10-Patch 实施与 qtest Study](../k230-spi-qspi-flash-window-study-plan.md)

Study 当前采用“Standard SPI → Standard SPI NOR → Dual/Quad QSPI → IRQ”
的顺序，并记录每个 Patch 的代码边界、对应 qtest、历史 RED 基线和分支重拆规则。

### 目标 MMIO 访问契约

下表给出依据本章 TRM/SDK 裁决后应达到的访问契约，不把它误写成当前 QEMU
已经完成的功能快照。当前实现差距和完成状态统一查阅 Study。普通 R/W 寄存器
最终应使用以下更新方式：
...
| 寄存器               | 目标 writable mask | 证据和说明                                                                                                                                                              | <br /> |
| ----------------- | ---------------: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :----- |
| `CTRLR0`          |     `0x01cf7f1f` | 保留 `DFS`、`SCPH/SCPOL`、`TMOD`、`SLV_OE`、`SRL`、`SSTE`、`CFS`、`SPI_FRF` 和 `SPI_HYPERBUS_EN`；bit 24 只保存读回、不产生 HyperBus 副作用；U-Boot 会写全 1 后回读以探测 `DFS32` 和 enhanced SPI 能力 | <br /> |
| `CTRLR1`          |     `0x0000ffff` | `NDF[15:0]`；SDK 和 U-Boot 都写 `frames - 1`                                                                                                                           | <br /> |
| `SSIENR`          |     `0x00000001` | 只保留 `SSIC_EN`，原有 CS/IRQ 更新行为不变                                                                                                                                     | <br /> |
| `MWCR`            |     `0x00000003` | 保留 TRM 明确为 RW 的 `MWMOD/MDD`；配置相关的 `MHS` 暂不开放                                                                                                                       | <br /> |
| `SER`             |    由 `num-cs` 决定 | 当前实例只有有效 CS 位可以写入                                                                                                                                                  | <br /> |
| `BAUDR`           |     `0x0000fffe` | 最低位只读为 0，软件写入后始终得到偶数分频值                                                                                                                                            | <br /> |
| `TXFTLR`          |     `0x07ff00ff` | `TFT[7:0]` 与 `TXFTHR[26:16]`；RT-Smart 会同时写两个字段                                                                                                                     | <br /> |
| `RXFTLR`          |     `0x000000ff` | K230 SDK 的 256 深度配置需要 8 位阈值                                                                                                                                        | <br /> |
| `IMR`             |     `0x000009bf` | 保留 TXE/TXO/RXU/RXO/RXF/MST/TXU、AXIE 和 DONE；排除 concurrent-XIP overflow、保留 bit 9 和 dynamic-wait error                                                                | <br /> |
| `RX_SAMPLE_DELAY` |     `0x000100ff` | SDK 实际写入 \`rdse << 16                                                                                                                                              | rdsd\` |
| `SPI_CTRLR0`      |     `0x6f3ffbbf` | 覆盖基础 XIP、增强 SPI、DTR/RXDS/CLK\_STRETCH 字段；bit 25/24 仅保存读回且不产生 HyperBus/Data Mask 线路副作用，真实综合状态待 `HW`                                                                 | <br /> |
| `DDR_DRIVE_EDGE`  |     `0x000000ff` | `TDE[7:0]`                                                                                                                                                         | <br /> |
| `XIP_MODE_BITS`   |     `0x0000ffff` | 仅 `has-xip=true` 的 FMC/spi0 保存并用于 XIP mode 阶段；QSPI profile RAZ/WI                                                                                                           | <br /> |
| `XIP_INCR_INST`   |     `0x0000ffff` | 仅 `has-xip=true` 的 FMC/spi0 保存并用于 XIP INCR 指令；QSPI profile RAZ/WI                                                                                                           | <br /> |
| `XIP_WRAP_INST`   |     `0x0000ffff` | 仅 `has-xip=true` 的 FMC/spi0 保存并用于 XIP WRAP 指令；QSPI profile RAZ/WI                                                                                                           | <br /> |
...
以下动态内容全部由 Study 单点维护：

- 当前 QEMU 分支的实现结论和关键差距；
- 10-Patch 主系列顺序和完成勾选；
- 46 项 qtest 的编号、归属、结果和运行命令；
- 当前提交历史和后续重拆策略。

本章后续仅在 TRM/SDK 证据发生变化时更新。实现进度或测试结果变化时，不在本章
复制第二份状态表。
