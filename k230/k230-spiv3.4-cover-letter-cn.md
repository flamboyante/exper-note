# K230 SPI v3.4 Cover Letter 中文版
...
~~~text
主题：[PATCH 00/11] hw/riscv：新增 K230 SPI、QSPI、IDMA 和 XIP 支持

您好：

当前 k230 机器已建模 C908 内核、CLINT、PLIC、看门狗和 UART，但尚无法
模拟真实启动路线。本系列新增 Kendryte K230 DesignWare SSI 控制器及其
机器集成，为之后完整启动做铺垫——使 U-Boot 能够通过标准 PIO、QSPI IDMA
和 XIP 读取窗口从 SPI NOR 启动。

K230 包含三个能力不同的 SSI 实例。SDK 按照地址映射对它们编号：spi0 是
SPI-OPI 实例，spi1 和 spi2 分别是 QSPI0 和 QSPI1。本系列依据 K230
Technical Reference Manual V0.3.1（2024-11-18）对寄存器访问约定、基于
FIFO 的标准 SPI 传输、中断路由、Dual/Quad SDR 传输、SPI NOR 挂接、同步
内部 DMA、HI_SYS SSI_CTRL 包装寄存器以及 spi0 XIP 只读窗口进行建模。
...
最后一个补丁新增 trace 事件，覆盖寄存器访问（排除 DR，因为每个 FIFO 帧都
经过 DR，记录它会在 U-Boot 和 Linux SPI NOR 传输期间淹没其他所有事件）、
事务边界、中断电平、IDMA 启动/完成/错误以及 XIP 读取。trace 点作为独立
补丁放在系列末尾，目的是让功能补丁聚焦于行为本身，trace 接口可以集中在一
处 review。这些事件仅观察模型行为，不改变其语义。
