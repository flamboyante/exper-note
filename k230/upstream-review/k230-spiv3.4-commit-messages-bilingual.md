...
本文按提交顺序整理 v3.4 的英文提交说明及中文对照。相对 v3.3，v3.4 保持 11 个补丁
的结构和顺序不变（trace 事件补丁仍在末尾），但对 commit 1/3/6/7/8/9/11 的正文做了
修订，并在 commit 2/7/8/10 中新增了对 `docs/system/riscv/k230.rst` 的修改。本文档
只记录现有 Git 提交信息，尚未对补丁本身做任何修改。

## 1. 12e325190a — hw/ssi: Add K230 DesignWare SSI register model
...
依据 K230 技术参考手册实现寄存器布局、复位值、可写位掩码、FIFO 状态、
片选 GPIO 和机器集成所需的 MMIO 区域。本补丁暂不实例化具体控制器。

Signed-off-by: Kangjie Huang <flamboyant.h.01@gmail.com>
~~~

## 2. 41750ee3b9 — hw/riscv/k230: Instantiate K230 SSI controllers
...
## 3. 6fc734d599 — hw/ssi: Implement K230 SSI FIFO and standard PIO transfers
...
依据 K230 TRM，TX FIFO 满时写入 DR 的数据被丢弃；读取时帧宽超过
DFS 字段的位被截断。禁用控制器会停止串行引擎，但保留 FIFO 中已
排队的帧。
...
## 4. 087a0aba5b — hw/ssi: Add K230 SSI interrupt controller
...
## 5. 662158aeeb — hw/riscv: Route K230 SSI IRQs to the PLIC
...
qtest 检查三个 SPI 实例的 TXE PLIC 路由，并验证 spi1 的 RXU 路由
和实例隔离。

Signed-off-by: Kangjie Huang <flamboyant.h.01@gmail.com>
~~~

## 6. db4d5698e1 — hw/ssi: Implement K230 enhanced QSPI transfers
...
路径。面向 SDK 的这条访问路径将在后续补丁中实现。

Signed-off-by: Kangjie Huang <flamboyant.h.01@gmail.com>
~~~

## 7. d42e5d4f87 — hw/riscv/k230: Attach SPI NOR flash to spi0
...
同时在 U-Boot 和 Linux 中通过命令行验证标准 SPI 读写。
...
## 8. 8f3b84c222 — hw/ssi: Implement K230 SSI internal DMA transfers
...
为 8 位 SDR Dual 和 Quad 传输模式实现同步内部 DMA。K230 SDK 驱动
在访问缓冲区前轮询 DONE，因此同步模型已足够，并可避免与客户机内存
竞争。仅支持 8 位 SDR Dual 和 Quad 模式，与该驱动实际使用的模式
一致。

从 SPIDR 和 SPIAR 构造增强命令，通过 AXIAR0/1 搬运数据，并通过
DONE 和 AXIE 中断原因上报完成或客户机内存访问失败。
...
## 9. 4b46245962 — hw/misc: Add K230 HI_SYS SSI control
...
控制器睡眠状态。qtest 覆盖寄存器复位值、可写掩码与读回行为、三
实例模式路由和睡眠状态切换。

Signed-off-by: Kangjie Huang <flamboyant.h.01@gmail.com>
~~~

## 10. 3556bbff1b — hw/ssi: Add K230 SSI XIP read window
...
## 11. 1342f13a6e — hw/ssi: Add trace events for K230 DesignWare SSI
...
DR 访问被排除在寄存器 trace 之外：每个 FIFO 帧都经过 DR，记录
DR 会在 U-Boot 和 Linux SPI NOR 传输期间淹没其他所有事件。
...
下表只列出 commit message 有变化的补丁，未列出的与 v3.3 完全一致：

| # | Commit   | 变化要点 |
|---|----------|----------|
| 1 | 12e325190a | 删除尾段 Machine integration and qtest coverage follow in later patches. Build-test the model with the riscv64-softmmu target.；改为以 based on the K230 Technical Reference Manual 引出依据 |
| 3 | 6fc734d599 | 正文重写：原段 Model DR aliases, frame-width truncation, loopback, FIFO capacity, dynamic status, receive backpressure, and controller disable semantics. 替换为更具体的 TRM 行为说明（TX FIFO 满时丢弃、DFS 截断、禁用后 FIFO 保留）；删除 Also exercise standard SPI reads and writes from U-Boot and Linux.（移到 commit 7） |
| 6 | db4d5698e1 | 删除尾句 Flash-backed PIO coverage follows once the machine attaches a SPI NOR device.（v3.4 中该 PIO Flash 覆盖已并入 commit 7） |
| 7 | d42e5d4f87 | 新增 Also exercise standard SPI reads and writes from U-Boot and Linux.（从 commit 3 移过来） |
| 8 | 8f3b84c222 | 新增首段说明：SDK 驱动轮询 DONE，同步模型足够；仅支持 8 位 SDR Dual/Quad，与驱动实际使用一致 |
| 9 | 4b46245962 | Cover the register contract 改为更具体的 Cover the register reset value, write mask, and read-back behaviour |
| 11| 1342f13a6e | 新增 DR 排除段落：DR accesses are excluded from register tracing: every FIFO frame passes through DR, so logging it would drown out everything else during U-Boot and Linux SPI NOR traffic. |

此外，commit 2/7/8/10 各新增 1 行对 docs/system/riscv/k230.rst 的修改（diffstat
相应增加 docs/system/riscv/k230.rst | 1 +）。

---

## 改动建议

下表汇总 v3.4 各 commit 当前状态及是否仍需修改：

| # | Commit                              | 是否需要改 | 说明 |
|---|-------------------------------------|------------|------|
| 1 | 12e325190a 寄存器模型                | 否         | 正文已精简，依据 TRM 表述清晰，保持现状即可 |
| 2 | 41750ee3b9 实例化控制器              | 否         | 无变化，保持现状即可 |
| 3 | 6fc734d599 FIFO 与标准 PIO           | 否         | v3.4 改写后正文具体引用 TRM 行为，比 v3.3 更清晰，保持现状即可 |
| 4 | 087a0aba5b 中断控制器                | 否         | 无变化，保持现状即可 |
| 5 | 662158aeeb PLIC 路由                 | 否         | 无变化，保持现状即可 |
| 6 | db4d5698e1 增强 QSPI                 | 否         | 删除冗余尾句后正文更紧凑，保持现状即可 |
| 7 | d42e5d4f87 挂接 SPI NOR              | 是（建议） | 正文 The system reaches a Linux shell. 与 cover letter 的 Linux initramfs shell 不一致，建议改为 The system reaches a Linux initramfs shell.，与 commit 8/10 的表述对齐 |
| 8 | 8f3b84c222 内部 DMA                  | 否         | v3.4 新增的同步模型说明很有价值，解释了为何同步实现足够，保持现状即可 |
| 9 | 4b46245962 HI_SYS 控制               | 否         | qtest 覆盖描述更具体，保持现状即可 |
| 10| 3556bbff1b XIP 读取窗口              | 否         | 无变化，保持现状即可 |
| 11| 1342f13a6e trace 事件                | 是（建议） | 见下文分析 |

### commit 11（trace 事件）分析

v3.4 在 commit message 中新增了 DR 排除段落，解释了为何不 trace DR 访问——这是
对 v3.3 review 反馈的正面回应。但 v3.3 文档中指出的结构性问题在 v3.4 中仍然
存在：

1. trace 调用仍追溯式插入：commit 11 仍在 `k230_dw_ssi.c` 中一次性插入 26 行
   trace 调用，覆盖 commit 1/3/4/8/10 的功能路径。
2. bisect 不友好：在 commit 1～10 之间 bisect 时 trace 不可用。
3. commit 11 仍位于系列末尾：trace 基础设施晚于所有功能补丁。

是否仍需修改取决于上游 review 的态度：

- 如果上游接受先实现、最后统一加 trace 的写法：则 commit 11 保持现状即可，
  v3.4 的 DR 排除说明已经足够。
- 如果上游要求 trace 与功能同补丁：则仍需按 v3.3 文档中的方案 A 拆分——
  commit 1 加 reg_read/write、commit 3 加 transaction、commit 4 加 irq_update、
  commit 8 加 idma_*、commit 10 加 xip_read，删除独立的 commit 11。

建议在 cover letter 中预先说明：trace 事件作为独立补丁放在系列末尾，目的是让
功能补丁的 diff 聚焦于行为本身，便于 review；trace 仅观察行为，不改变语义。这样
即使上游倾向拆分，也能先看到作者的选择理由。

### cover letter（非 commit，但配套需要更新）

v3.2 的 cover letter 仍是 10 补丁版本，v3.4 需要单独更新（已完成，见
`k230-spiv3.4-cover-letter.md` 和 `k230-spiv3.4-cover-letter-cn.md`）：

- 主题改为 [PATCH 00/11]
- 补丁列表加入 hw/ssi: Add trace events for K230 DesignWare SSI
- diffstat 加入 hw/ssi/trace-events（11 行）和 k230_dw_ssi.c（26 行），以及
  commit 2/7/8/10 新增的 docs/system/riscv/k230.rst（每条 +1 行，共 4 行）
- 测试小节补一句说明 trace 在 U-Boot/Linux SPI NOR 传输期间可用，且 DR 访问被
  排除以避免淹没日志
- 在 cover letter 中预先解释 trace 补丁独立放置的理由（已完成）
