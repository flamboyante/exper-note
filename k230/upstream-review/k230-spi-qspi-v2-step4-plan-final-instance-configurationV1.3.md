...
V1.3 修订：2026-07-30（未实现扩展寄存器采用固定 unsupported 语义，功能配置随真实消费者引入）
...
> **上游投稿边界说明（2026-07-30）**：第一批提交通用 DW SSI 的标准寄存器版图、Standard SPI PIO、基础 IRQ、K230 三实例与 PLIC 集成，以及独立的 Standard 1-1-1 SPI NOR 挂接。external DMA 寄存器地址保留，但读返回 0、写忽略并记录 unsupported；enhanced SPI、IDMA engine、HI_SYS 和 XIP 分批后送。第一批不增加 `has-enhanced-spi`、`has-idma`、`has-xip`、`xip-window-size` 或内部 capability 位图。
...
2. Step 4.1：建立 `DwSsiConfig`、第一批所需 properties、配置校验、动态 FIFO 和迁移一致性检查；
3. Step 4.2：在 K230 machine 中显式应用 QSPI0、QSPI1、SPI-OPI/FMC 三个 profile，消除实例参数和 XIP region 的 K230 硬编码推断；
4. Step 4.3：锁定未实现 enhanced、DMA/IDMA 和 XIP 寄存器的固定语义，以及对应数据路径、IRQ 和资源边界；
5. Step 4.4：完成通用模型、K230 集成、公共头文件、迁移状态和未来 patch 归属检查。

本步不实现 enhanced、DDR、RXDS、Octal、XIP、internal DMA engine 或 external DMA request 行为。第一批数据路径只执行 Standard single-line SPI；后续 series 在引入真实功能时扩展相应配置和运行状态。
...
| H8 | `hw/ssi/dw_ssi.c:813-984` | DMA 寄存器和 IDMA engine 被同一逻辑隐式绑定 | external DMA 地址保留并固定 unsupported；internal DMA engine、guest memory 和 DONE/AXIE 随 IDMA series 引入；TXU 保持基础 FIFO IRQ |
...
| `hw/i2c/designware_i2c.c:597-605` | DMA 地址存在，但 read 返回 0、write 忽略并记录 unsupported | external DMA 寄存器采用同类固定语义，不保存 readback |
...
| DMA 寄存器 | external DMA 地址保留，read 返回 0、write 忽略；internal-AXI 配置和 engine 随 IDMA series 引入 |
| IRQ / VMState | TXU 属于基础 FIFO IRQ，DONE/AXIE 第一批不产生；迁移只对 FIFO 深度做 equality |
...
2. external DMA、enhanced、IDMA、XIP 已知 offset：read 返回 0，write 忽略并记录 unsupported；
...
### 4.2 external DMA 寄存器

DesignWare SSI 的标准 external DMA 地址继续列入寄存器目录：
...
- read 始终返回 0；
- write 不改变 `regs[]`；
- 不保存 threshold 或 enable；
- 不产生 TX/RX DMA request；
- 不访问 guest memory；
- 不进入 VMState；
- 不需要 `dma-register-layout`。

如果后续 external DMA series 实现 request 接口，再把这三个 offset 从 unsupported 分派移入真实寄存器语义，并同时增加 request wiring、状态和 qtest。

### 4.3 enhanced 与 XIP 寄存器
...
| external DMA | 地址识别，read 0、write ignore | 无 DMA request IRQ | 不保存寄存器值 | 无 DMA link |
...
external DMA series 只实现 request 接口和 `DMACR/DMATDLR/DMARDLR`；internal DMA series 才需要 internal-AXI layout、guest-memory 进度与 DONE/AXIE。
...
引入配置表达、动态 FIFO 和通用测试载体。通用默认实例提供 Standard PIO/FIFO/基础 IRQ；external DMA、enhanced、IDMA 和 XIP 寄存器使用固定 unsupported/RAZ-WI 语义。

### TDD 顺序

1. 新增独立 `dw-ssi-test` machine 和 qtest 骨架；
2. 先写 property 默认值、`fifo-depth=8`、unsupported 寄存器和非法取值测试；
...
- external DMA、enhanced、IDMA、XIP unsupported read/write 测试通过；
- `DwSsiState` 保留 Standard regs、FIFO、phase、IRQ latch 和 active CS；
...
- 三实例 external DMA、enhanced、IDMA、XIP offset 使用相同固定 unsupported 语义；
- 三实例都只创建和映射 region 0，`0xc0000000` 不映射；
- Standard PIO/IRQ 和寄存器兼容性测试通过。

---

## 10. Step 4.3：锁定未实现扩展的固定语义

第一批不建立 capability property 或内部位图。测试直接验证寄存器 read/write、数据路径、IRQ output 和资源。

### 10.1 enhanced SPI 寄存器
...
- `DMACR`、`DMATDLR`、`DMARDLR` read 返回 0、write 忽略；
- `AXIAWLEN`、`AXIARLEN`、`SPIDR`、`SPIAR`、`AXIAR0/1`、`AXIECR`、`DONECR` 同样 read 0、write ignore；
- 向 IMR 写 DONE/AXIE 位，读回仍为 0；TXU 位仍可写入和读回；
- 尝试写 DMA/IDMA 寄存器后不访问 guest memory；
- `SR.CMPLTD_DF == 0`；
- 用 `qtest_irq_intercept_out()` 拦截 `/machine/dw-ssi`，确认设备只暴露七路基础 IRQ；再制造 TX FIFO underflow，确认 TXU raw status 和输出仍可见；
- DR 写入继续进入普通 TX FIFO，证明关闭 IDMA 没有破坏 PIO。

#### 实现要求

- external DMA 和 internal-AXI 地址进入集中 unsupported helper；
...
- enhanced、external DMA、internal DMA/IDMA 和 XIP unsupported 行为都有测试；
...
QOM properties 在 realize 前确定，和 machine type/profile 一起由源端、目的端分别创建。它们不作为 guest 可变状态迁移；`fifo-depth` 会改变 FIFO 状态解释，必须作为 equality guard 放在依赖字段之前：

```c
VMSTATE_UINT32_EQUAL(cfg.fifo_depth, DwSsiState),
```

`VMSTATE_FIFO32` 使用目的端已经创建的 FIFO capacity 解释数据，因此 `fifo-depth` 不一致必须拒绝。第一批没有 DMA layout、enhanced/IDMA 或 XIP 配置；`num-cs`、ID、VERSION 和 reset 值不在本步增加 equality。
...
当前 V2 分支尚未上游发布，Step 4 不承诺与未发布中间态进行跨版本迁移。Plan Final V1.3 只加入 FIFO 深度 equality，不把全部 cfg 参数塞进迁移流。测试覆盖同 profile 成功和 FIFO 深度不一致失败。
...
| `/dw-ssi/register/unsupported-dma` | 默认通用实例 | external/internal DMA offset read 0/write ignore，无 guest memory/DONE/AXIE |
| `/dw-ssi/register/unsupported-xip` | 默认通用实例 | XIP-only 寄存器 RAZ/WI，无 GPIO 和第二 region |
| `/dw-ssi/migration/same-profile` | 相同 FIFO | 迁移成功，FIFO、寄存器和 IRQ 状态恢复 |
...
- [ ] `DwSsiState` 保留完整 Standard regs、FIFO、phase、IRQ latch 和 active CS
...
- [ ] 增加 `fifo-depth` VMState equality
- [ ] 同 profile 迁移成功，FIFO 深度不一致迁移失败
- [ ] external DMA、enhanced、IDMA、XIP 地址走 unsupported/RAZ-WI 测试
...
- [ ] 三实例 reset、SER、FIFO、unsupported/property 测试通过

### Step 4.3：未实现扩展语义

- [ ] SPI_FRF 保持 Standard，enhanced offset read 0/write ignore
- [ ] external DMA 和 internal-AXI offset read 0/write ignore
- [ ] 无 guest memory 访问、DMA request 和 engine 行为
...
3. **地址与实现分离**：external DMA 和其他扩展 offset 可以存在于寄存器目录中；第一批 read 0、write ignore，不保存状态；
...
- 基础 VMState：寄存器、FIFO、PIO phase、remaining frames、IRQ latch、active CS，以及 FIFO equality；
...
2. DMA/IDMA：external request 接口与 optional internal DMA engine、guest memory 搬运、DONE/AXIE；
