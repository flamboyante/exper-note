...
本节以 TRM 12.3 的寄存器表为骨架，直接合并 TRM 5.3 FMC、K230 SDK 和 QEMU 实现结论。`中文说明` 保留原寄存器用途；`K230 采用与访问实现`、`等级` 和 `依据` 是学习与实现补充，不是 TRM 原始表头。

实现等级统一为：`A` 表示必须完整实现行为，`B` 表示先实现掩码与读回、复杂行为可后补，`C` 表示固定值/动态只读/读取清除，`D` 表示当前 K230 profile 按 RAZ/WI，`E` 表示属于 SPI 控制器之外的 SoC 包装寄存器。

| 偏移地址 | 寄存器 | 中文说明 | K230 采用与访问实现 | 等级 | 依据 |
|---|---|---|---|---:|---|
| `0x000` | `CTRLR0` | 控制寄存器 0 | 可写掩码 `0x00c06fdf`；`DFS/TMOD/SPI_FRF/SCPOL/SCPH/SSTE` 进入事务行为 | `A/B` | `T12-R`、`RT/UB/LX` |
| `0x004` | `CTRLR1` | 控制寄存器 1 | `NDF[15:0]` 可写，决定 `NDF+1` 帧 | `A` | `T12-R`、`RT/UB/LX` |
| `0x008` | `SSIENR` | SSI 使能寄存器 | 仅 bit 0；清零停止传输并清空 TX/RX FIFO | `A` | `T12-F/T12-R` |
| `0x00c` | `MWCR` | Microwire 控制寄存器 | K230 SPI profile 不实现 Microwire，RAZ/WI | `D` | `T12-R`、SDK 无 Microwire 路径 |
| `0x010` | `SER` | 从设备使能寄存器 | `num-cs=1`，仅 bit 0 可写 | `A` | `DTS`、`RT` |
| `0x014` | `BAUDR` | 波特率选择寄存器 | 可写 `0x0000fffe`；bit 0 固定为 0，值 0 禁止输出串行时钟 | `A` | `T12-F/T12-R` |
| `0x018` | `TXFTLR` | TX FIFO 水位阈值寄存器 | `TXFTHR[26:16]`、`TFT[7:0]`，可写 `0x07ff00ff` | `A` | `T12-R`、`RT` |
| `0x01c` | `RXFTLR` | RX FIFO 水位阈值寄存器 | `RFT[7:0]`，可写 `0xff` | `A` | `T12-R`、`RT` |
| `0x020` | `TXFLR` | TX FIFO 水位寄存器 | 动态返回 256 项 TX FIFO 当前数量，需 9 位表示 0–256 | `C` | `T12-R`、`RT` |
| `0x024` | `RXFLR` | RX FIFO 水位寄存器 | 动态返回 256 项 RX FIFO 当前数量 | `C` | `T12-R`、`RT` |
| `0x028` | `SR` | 状态寄存器 | 根据 FIFO、BUSY、错误和 IDMA 完成帧数动态计算 | `C` | `T12-R` |
| `0x02c` | `IMR` | 中断屏蔽寄存器 | 有效掩码 `0x9bf`；复位裁决为 `0x3f` | `A` | `T12-R/T53-R`、`RT/DTS` |
| `0x030` | `ISR` | 中断状态寄存器 | 动态返回 `RISR & IMR & 0x9bf` | `C` | `T12-R` |
| `0x034` | `RISR` | 原始中断状态寄存器 | 由 FIFO 水位和锁存事件产生，有效掩码 `0x9bf` | `C` | `T12-R`、`RT/DTS` |
| `0x038` | `TXEICR` | TX FIFO 错误中断清除寄存器 | 读取清除 TXO/TXU，写入忽略 | `C` | `T12-R` |
| `0x03c` | `RXOICR` | RX FIFO 溢出中断清除寄存器 | 读取清除 RXO，写入忽略 | `C` | `T12-R` |
| `0x040` | `RXUICR` | RX FIFO 下溢中断清除寄存器 | 读取清除 RXU，写入忽略 | `C` | `T12-R` |
| `0x044` | `MSTICR` | 多主机中断清除寄存器 | 读取清除 MST，写入忽略 | `C` | `T12-R` |
| `0x048` | `ICR` | 中断清除寄存器 | 读取清除 TXO/RXU/RXO/MST；TXU 由 `TXEICR` 清除 | `C` | `T12-R` |
| `0x04c` | `DMACR` | DMA 控制寄存器 | 内部 DMA profile；可写 `0x0007ff5c`，`IDMAE/AINC/ATW` 参与搬运 | `A/B` | `T12-F/T12-R`、`RT/UB/LX` |
| `0x050` | `DMATDLR` | 外部 DMA 发送水位寄存器 | `SSIC_HAS_DMA=2`，当前 K230 不采用，RAZ/WI | `D` | `RT/UB` |
| `0x050` | `AXIAWLEN` | 内部 DMA目的端 burst 长度 | `AWLEN[15:8]` 可写，mask `0xff00` | `A` | `T12-R`、`UB/LX` |
| `0x054` | `DMARDLR` | 外部 DMA 接收水位寄存器 | `SSIC_HAS_DMA=2`，当前 K230 不采用，RAZ/WI | `D` | `RT/UB` |
| `0x054` | `AXIARLEN` | 内部 DMA源端 burst 长度 | `ARLEN[15:8]` 可写，mask `0xff00` | `A` | `T12-R`、`UB/LX` |
| `0x058` | `IDR` | 标识寄存器 | 固定只读 `0xa1b2c3d5` | `C` | `T12-R/T53-R` |
| `0x05c` | `SSIC_VERSION_ID` | 组件版本寄存器 | 固定只读 `0x3130332a` | `C` | `T12-R/T53-R` |
| `0x060–0x0ec` | `DR0–DR35` | 数据寄存器 | 36 个地址均为同一 FIFO 数据口别名；按 `DFS+1` 截断并右对齐 | `A` | `T12-R`、`RT dr[36]` |
| `0x068` | `SSI_CTRL` | TRM 混入的 SoC 控制寄存器 | 不属于 SPI `base+0x068`；实际地址 `0x91585068`，SPI 此偏移是 `DR2` | `E` | `MAP`、`UB`、`RT` |
| `0x0f0` | `RX_SAMPLE_DELAY` | RX 采样延迟寄存器 | `SE[16]`、`RSD[7:0]` 可写，mask `0x000100ff` | `A` | `T12-R`、`RT/UB/LX` |
| `0x0f4` | `SPI_CTRLR0` | SPI 控制寄存器 0 | 可写 `0x6c3ffbbf`；`spi0` 复位 `0x28000200`，`spi1/2` 复位 `0x04000200` | `A/B` | `T12-R/T53-R`、SDK |
| `0x0f8` | `DDR_DRIVE_EDGE` | 发送驱动边沿寄存器 | 低 8 位可写；先保证掩码与读回 | `B` | `T12-R/T53-R`、`UB/LX` |
| `0x0fc` | `XIP_MODE_BITS` | 5.3 XIP mode bits | 低 16 位；`spi0` 参与 XIP，`spi1/2` 仅保留读回 | `A/B` | `T53-R`、`RT` |
| `0x100` | `XIP_INCR_INST` | 5.3 XIP INCR 指令 | 低 16 位；`spi0` XIP 指令 | `A/B` | `T53-R`、`RT` |
| `0x104` | `XIP_WRAP_INST` | 5.3 XIP WRAP 指令 | 低 16 位；`spi0` XIP 指令 | `A/B` | `T53-R`、`RT` |
| `0x108` | `XIP_CTRL` | 5.3 concurrent XIP 控制 | 当前 `SSIC_CONCURRENT_XIP_EN` 未启用，RAZ/WI | `D` | `T53-R`、`RT` 条件布局 |
| `0x10c` | `XIP_SER` | 5.3 XIP 片选 | 当前 concurrent XIP profile 未启用，RAZ/WI | `D` | `T53-R`、`RT` |
| `0x110` | `XRXOICR` | 5.3 XIP RX overflow 清除 | 当前无 XRXO IRQ，RAZ/WI | `D` | `T53-R`、`RT/DTS` |
| `0x114` | `XIP_CNT_TIME_OUT` | 5.3 连续 XIP 超时 | 当前 concurrent XIP profile 未启用，RAZ/WI | `D` | `T53-R`、`RT` |
| `0x118` | `SPI_CTRLR1` | 动态等待状态控制 | SDK 将该地址保留，RAZ/WI | `D` | `T12-R/T53-R`、`RT` |
| `0x11c` | `SPITECR` | SPI 发送错误清除寄存器 | 动态等待状态未集成，RAZ/WI | `D` | `T12-R/T53-R`、`RT` |
| `0x120` | `SPIDR` | 内部 DMA SPI 指令寄存器 | 低 16 位可写并参与 IDMA 事务 | `A` | `T12-R/T53-R`、`RT/UB/LX` |
| `0x124` | `SPIAR` | 内部 DMA SPI 地址寄存器 | 32 位可写并参与 IDMA 事务 | `A` | `T12-R/T53-R`、`RT/UB/LX` |
| `0x128` | `AXIAR0` | 内部 DMA 内存地址低位 | 与 `AXIAR1` 组合成 64 位 guest physical address | `A` | `T12-R`、`RT/UB/LX` |
| `0x12c` | `AXIAR1` | 内部 DMA 内存地址高位 | 当前按高 32 位实现 | `A` | `T12-R`、`RT` |
| `0x130` | `AXIECR` | AXI Master 错误中断清除 | 读取清除 AXIE，写入忽略 | `C` | `T12-R`、`RT/UB/LX` |
| `0x134` | `DONECR` | 传输完成中断清除 | 读取清除 DONE，写入忽略 | `C` | `T12-R`、`RT/UB/LX` |
| `0x138–0x13c` | 保留 | SDK 结构体保留区 | RAZ/WI | `D` | `RT` |
| `0x140` | `XIP_WRITE_INCR_INST` | 5.3 XIP write INCR 指令 | `SSIC_XIP_WRITE_REG_EN` 未启用，RAZ/WI | `D` | `T53-R`、`RT` |
| `0x144` | `XIP_WRITE_WRAP_INST` | 5.3 XIP write WRAP 指令 | `SSIC_XIP_WRITE_REG_EN` 未启用，RAZ/WI | `D` | `T53-R`、`RT` |
| `0x148` | `XIP_WRITE_CTRL` | 5.3 XIP write 控制 | `SSIC_XIP_WRITE_REG_EN` 未启用，RAZ/WI | `D` | `T53-R`、`RT` |
...
- **字段不存在**：例如当前 K230 profile 的 HyperBus、动态等待状态和 XIP write 扩展，采用 RAZ/WI。
...
|---|---|---|---|---|---:|---|---|
| 31 | 随配置变化 | `SSI_IS_MST` | `0x0` | 选择 SPI 工作在 Master 还是 Slave：`1=MASTER`，`0=SLAVE`。 | `D` | 当前 K230 profile 未集成或未采用该字段，RAZ/WI。 | `T12-R` |
| 30:26 | R | `RSVD_CTRLR0_26_31` | `0x0` | 保留位，读取为 0。 | `D` | 保留位：读取为 0，写入忽略，不进入普通寄存器存储。 | `T12-R` |
| 25 | 随配置变化 | `SPI_DWS_EN` | `0x0` | SPI 动态等待状态使能：`0=DISABLE`，`1=ENABLE`。 | `D` | 当前 K230 profile 未集成或未采用该字段，RAZ/WI。 | `T12-R` |
| 24 | 随配置变化 | `SPI_HYPERBUS_EN` | `0x0` | HyperBus 帧格式使能：`0=DISABLE`，`1=ENABLE`。 | `D` | 当前 K230 profile 未集成或未采用该字段，RAZ/WI。 | `T12-R` |
...
| 19:16 | R/W | `CFS` | `0x0` | Control Frame Size。 | `D` | 当前 K230 profile 未集成或未采用该字段，RAZ/WI。 | `T12-R` |
| 15 | R | `RSVD_CTRLR0_15` | `0x0` | 保留位，读取为 0。 | `D` | 保留位：读取为 0，写入忽略，不进入普通寄存器存储。 | `T12-R` |
| 14 | R/W | `SSTE` | `0x1` | 从设备选择翻转使能。`1=TOGGLE_EN`：相邻帧之间翻转 `ss_n`，`ss_n` 为高时 `sclk` 保持默认值；`0=TOGGLE_DISABLE`：传输期间 `ss*_n` 保持低电平，`sclk` 连续运行。 | `A` | 按字段掩码保存，并依照 TRM 语义参与 K230 SPI/IDMA 数据路径。 | `T12-R` |
| 13 | 随配置变化 | `SRL` | `0x0` | Shift Register Loop，仅用于测试：`1=TESTING_MODE`，`0=NORMAL_MODE`。 | `B` | 保存和读回；实现 Linux SPI_LOOP 时再接入内部回环数据路径。 | `T12-R` |
| 12 | R/W | `SLV_OE` | `0x0` | Slave Output Enable：`1=DISABLED`，`0=ENABLED`。 | `D` | 当前 K230 profile 未集成或未采用该字段，RAZ/WI。 | `T12-R` |
...
| 7:6 | 随配置变化 | `FRF` | `0x0` | Frame Format：TRM 本章列出 `0=SPI`，即 Motorola SPI Frame Format。 | `A` | 仅 FRF=0 进入 K230 Motorola SPI 数据路径，其他编码不得复用 SPI 行为。 | `T12-R` |
...
| 2 | 随配置变化 | `MHS` | `0x0` | Microwire Handshaking：`1=ENABLED`，`0=DISABLE`。 | `D` | 当前 K230 profile 未集成或未采用该字段，RAZ/WI。 | `T12-R` |
| 1 | R/W | `MDD` | `0x0` | Microwire 数据方向：`1=TRANSMIT`，SSI 发送数据；`0=RECEIVE`，SSI 接收数据。 | `D` | 当前 K230 profile 未集成或未采用该字段，RAZ/WI。 | `T12-R` |
| 0 | R/W | `MWMOD` | `0x0` | Microwire 传输模式：`1=SEQUENTIAL`，`0=NON_SEQUENTIAL`。 | `D` | 当前 K230 profile 未集成或未采用该字段，RAZ/WI。 | `T12-R` |
...
| 位 | 访问 | 名称 | 复位值 | 中文说明 | K230等级 | K230实现注释 | 依据 |
|---|---|---|---|---|---:|---|---|
| 31:y | R | `RSVD_SER` | `0x0` | 保留位，读取为 0。 | `D` | 保留位：读取为 0，写入忽略，不进入普通寄存器存储。 | `T12-R/DTS/RT` |
| x:0 | R/W | `SER` | `0x0` | Slave Select Enable Flag。每一位对应 SPI Master 的一条 `ss_x_n`。置 1 后，对应从设备选择线会在串行传输开始时有效。开始传输前，应置位目标从设备对应的位；非 broadcast 模式下，该字段只应有一位置 1。对该寄存器置位或清零不会在当前传输中立即改变对应输出，而是在后续传输边界生效。 | `A` | DTS 给出 num-cs=1，仅 bit 0 有效，其余片选位 RAZ/WI。 | `T12-R/DTS/RT` |
...
| 10 | 随配置变化 | `SPITEM` | `0x0` | `ssi_spite_intr`（SPI transmit error）屏蔽控制。 | `D` | 当前 K230 profile 未集成或未采用该字段，RAZ/WI。 | `T12-R/DTS/RT` |
...
| 6 | 随配置变化 | `XRXOIM` | `0x0` | `ssi_xrxo_intr`（XIP RX FIFO overflow）屏蔽控制。 | `D` | 当前 K230 profile 未集成或未采用该字段，RAZ/WI。 | `T12-R/DTS/RT` |
...
| 10 | R | `SPITES` | `0x0` | `ssi_spite_intr` 屏蔽后的状态。 | `D` | 当前 K230 profile 未集成或未采用该字段，RAZ/WI。 | `T12-R/DTS/RT` |
...
| 6 | R | `XRXOIS` | `0x0` | `ssi_xrxo_intr` 屏蔽后的状态。 | `D` | 当前 K230 profile 未集成或未采用该字段，RAZ/WI。 | `T12-R/DTS/RT` |
...
| 10 | R | `SPITER` | `0x0` | SPI transmit error 原始状态。若 SPI Master 在 `SPI_CTRLR1.MAX_WS` 指定时间内始终无法从 Slave 获得 READY，控制器停止 SPI 传输；写操作中还会清空 FIFO。 | `D` | 当前 K230 profile 未集成或未采用该字段，RAZ/WI。 | `T12-R/DTS/RT` |
| 9 | R | `RSVD_9_RISR` | `0x0` | 保留位，读取为 0。 | `D` | 保留位：读取为 0，写入忽略，不进入普通寄存器存储。 | `T12-R/DTS/RT` |
...
| 6 | R | `XRXOIR` | `0x0` | `ssi_xrxo_intr` 的原始状态。 | `D` | 当前 K230 profile 未集成或未采用该字段，RAZ/WI。 | `T12-R/DTS/RT` |
...
| 位 | 访问 | 名称 | 复位值 | 中文说明 | K230等级 | K230实现注释 | 依据 |
|---|---|---|---|---|---:|---|---|
| 31:y | R | `RSVD_DMACR` | `0x0` | 保留位，读取为 0。 | `D` | 保留位：读取为 0，写入忽略，不进入普通寄存器存储。 | `T12-R/RT/UB/LX` |
| x:15 | 随配置变化 | `AID` | `0x0` | AXI `awid/arid` 信号值。 | `B` | 实现写掩码、保存和读回；复杂时序或总线属性可后补。 | `T12-R/RT/UB/LX` |
| 14:12 | 随配置变化 | `APROT` | `0x0` | AXI `arprot/awprot` 信号值。 | `B` | 实现写掩码、保存和读回；复杂时序或总线属性可后补。 | `T12-R/RT/UB/LX` |
| 11:8 | 随配置变化 | `ACACHE` | `0x0` | AXI `arcache/awcache` 信号值。 | `B` | 实现写掩码、保存和读回；复杂时序或总线属性可后补。 | `T12-R/RT/UB/LX` |
| 7 | R | `RSVD_DMACR7` | `0x0` | 保留位，读取为 0。 | `D` | 保留位：读取为 0，写入忽略，不进入普通寄存器存储。 | `T12-R/RT/UB/LX` |
| 6 | 随配置变化 | `AINC` | `0x0` | AXI 地址模式：`1=Increment`，`0=No Change`。递增时将地址对齐到下一个 `DMACR.ATW` 边界。 | `A` | K230 SDK 采用 1=Increment；IDMA 每个 beat 后按 ATW 递增地址。 | `T12-R/RT/UB/LX` |
| 5 | R | `RSVD_DMACR5` | `0x0` | 保留位，读取为 0。 | `D` | 保留位：读取为 0，写入忽略，不进入普通寄存器存储。 | `T12-R/RT/UB/LX` |
| 4:3 | 随配置变化 | `ATW` | `0x0` | DMA 的 AXI 传输宽度，映射到 `arsize/awsize`，且不得大于 `SSIC_AXI_DW`：`0=1 byte`，`1=2 bytes`，`2=4 bytes`，`3=8 bytes`。 | `A` | 决定 AXI beat 字节数，不是 SPI 帧宽；SDK 使用 3=8 bytes。 | `T12-R/RT/UB/LX` |
| 2 | 随配置变化 | `IDMAE` | `0x0` | Internal DMA Enable。仅应在 `CTRLR0.FRF=0` 且 `CTRLR0.SPI_FRF>0` 时使能。 | `A` | K230 选择 SSIC_HAS_DMA=2，置位后进入内部 AXI DMA 路径。 | `T12-R/RT/UB/LX` |
| 1 | 随配置变化 | `TDMAE` | `0x0` | TX FIFO DMA 通道使能：`1=ENABLED`，`0=DISABLE`。 | `D` | 当前 K230 profile 未集成或未采用该字段，RAZ/WI。 | `T12-R/RT/UB/LX` |
| 0 | 随配置变化 | `RDMAE` | `0x0` | RX FIFO DMA 通道使能：`1=ENABLED`，`0=DISABLE`。 | `D` | 当前 K230 profile 未集成或未采用该字段，RAZ/WI。 | `T12-R/RT/UB/LX` |
...
| x:0 | R/W | `DMATDL` | `0x0` | TX DMA 请求水位。当 `TDMAE=1` 且 TX FIFO 有效项数量小于或等于 `DMATDL` 时，产生 `dma_tx_req`。 | `D` | 当前 K230 profile 不采用该寄存器能力，读取为 0、写入忽略。 | `T12-R` |
...
| 位 | 访问 | 名称 | 复位值 | 中文说明 | K230等级 | K230实现注释 | 依据 |
|---|---|---|---|---|---:|---|---|
| 31:y | R | `RSVD_AXIAWLEN2` | `0x0` | 保留位，读取为 0。 | `D` | 保留位：读取为 0，写入忽略，不进入普通寄存器存储。 | `T12-R/RT/UB/LX` |
| x:8 | R/W | `AWLEN` | `0x7` | Destination Burst Length。 | `A` | K230 按 bits[15:8]，可写 mask 0xff00。 | `T12-R/RT/UB/LX` |
| 7:0 | R | `RSVD_AXIAWLEN` | `0x0` | 保留位，读取为 0。 | `D` | 保留位：读取为 0，写入忽略，不进入普通寄存器存储。 | `T12-R/RT/UB/LX` |
...
| x:0 | R/W | `DMARDL` | `0x0` | RX DMA 请求水位为 `DMARDL + 1`。当 `RDMAE=1` 且 RX FIFO 有效项数量大于或等于 `DMARDL + 1` 时，产生 `dma_rx_req`。 | `D` | 当前 K230 profile 不采用该寄存器能力，读取为 0、写入忽略。 | `T12-R` |
...
| 位 | 访问 | 名称 | 复位值 | 中文说明 | K230等级 | K230实现注释 | 依据 |
|---|---|---|---|---|---:|---|---|
| 31:y | R | `RSVD_AXIARLEN2` | `0x0` | 保留位，读取为 0。 | `D` | 保留位：读取为 0，写入忽略，不进入普通寄存器存储。 | `T12-R/RT/UB/LX` |
| x:8 | R/W | `ARLEN` | `0x7` | Source Burst Length。 | `A` | K230 按 bits[15:8]，可写 mask 0xff00。 | `T12-R/RT/UB/LX` |
| 7:0 | R | `RSVD_AXIARLEN` | `0x0` | 保留位，读取为 0。 | `D` | 保留位：读取为 0，写入忽略，不进入普通寄存器存储。 | `T12-R/RT/UB/LX` |
...
| 位 | 访问 | 名称 | 复位值 | 中文说明 | K230等级 | K230实现注释 | 依据 |
|---|---|---|---|---|---:|---|---|
| 17:13 | R/W | `rxds_delay_num` | `0x2` | `rxds` 延迟线配置：`bit[0]` 选择边沿，`0=negedge`、`1=posedge`；`bit[3:1]` 为延迟数量。 | `E` | TRM 位范围与 SDK 冲突；按 U-Boot 自洽布局实现为 bits[17:14] R/W。 | `T12-R/UB/MAP` |
| 17:13 | RO | `ssi2_spi_mode` | `0x0` | `ssi2` SPI 模式状态。 | `E` | TRM 与 rxds_delay_num 重叠；按 U-Boot 布局实现为 bits[12:11] RO。 | `T12-R/UB/MAP` |
| 10 | RO | `ssi2_ssi_sleep` | `0x0` | `ssi2` sleep 状态。 | `E` | HI_SYS_CONFIG 只读状态，根据对应实例禁用和空闲状态产生。 | `T12-R/UB/MAP` |
| 9:8 | RO | `ssi1_spi_mode` | `0x0` | `ssi1` SPI 模式状态。 | `E` | HI_SYS_CONFIG 只读状态，反映对应实例当前 SPI_FRF/profile。 | `T12-R/UB/MAP` |
| 7 | RO | `ssi1_ssi_sleep` | `0x0` | `ssi1` sleep 状态。 | `E` | HI_SYS_CONFIG 只读状态，根据对应实例禁用和空闲状态产生。 | `T12-R/UB/MAP` |
| 6:5 | RO | `ssi0_spi_mode` | `0x0` | `ssi0` SPI 模式状态。 | `E` | HI_SYS_CONFIG 只读状态，反映对应实例当前 SPI_FRF/profile。 | `T12-R/UB/MAP` |
| 4 | RO | `ssi0_ssi_sleep` | `0x0` | `ssi0` sleep 状态。 | `E` | HI_SYS_CONFIG 只读状态，根据对应实例禁用和空闲状态产生。 | `T12-R/UB/MAP` |
| 3:2 | R | 未命名保留位 | `0x0` | 保留位，读取为 0。 | `E` | HI_SYS_CONFIG 包装位；按 U-Boot 位图处理，保留位读零。 | `T12-R/UB/MAP` |
| 0 | R/W | `ssi0_xip_en` | `0x0` | `ssi0` XIP 使能。 | `E` | 实际地址 0x91585068；控制 spi0 XIP window，不属于 SPI base+0x68。 | `T12-R/UB/MAP` |

> [!NOTE]
> **原文译注：** `SSI_CTRL` 表中 `rxds_delay_num` 和 `ssi2_spi_mode` 都标为位 `17:13`，同时未列出若干中间位。本文原样保留该重叠和缺项，不能据此表单独确定最终 RTL 位图。
...
| 位 | 访问 | 名称 | 复位值 | 中文说明 | K230等级 | K230实现注释 | 依据 |
|---|---|---|---|---|---:|---|---|
| 31 | R | `RSVD_SPI_CTRLR0` | `0x0` | 保留位，读取为 0。 | `D` | 保留位：读取为 0，写入忽略，不进入普通寄存器存储。 | `T12-R/T53-R/RT/UB/LX` |
| 30 | 随配置变化 | `CLK_STRETCH_EN` | `0x0` | SPI clock stretching 使能。写操作中 TX FIFO 变空时暂停时钟，直到 FIFO 有足够数据；读操作中 RX FIFO 变满时暂停时钟，直到软件读走数据。 | `A` | Linux 增强 SPI 路径会置位；FIFO 空/满时影响串行时钟推进。 | `T12-R/T53-R/RT/UB/LX` |
| 29 | 随配置变化 | `XIP_PREFETCH_EN` | `0x0` | XIP 预取使能。预取下一个连续地址的数据帧以降低连续访问延迟；若下一次 XIP 请求不连续，则丢弃预取数据。 | `B` | 实现写掩码、保存和读回；复杂时序或总线属性可后补。 | `T12-R/T53-R/RT/UB/LX` |
| 28 | R | `RSVD_SPI_CTRLR0_28` | `0x0` | 保留位，读取为 0。 | `D` | 保留位：读取为 0，写入忽略，不进入普通寄存器存储。 | `T12-R/T53-R/RT/UB/LX` |
| 27:26 | 随配置变化 | `XIP_MBL` | `0x1` | XIP mode bits 长度，仅在 `XIP_MD_BIT_EN=1` 时有效：`0=2 bits`，`1=4 bits`，`2=8 bits`，`3=16 bits`。 | `A/B` | spi0 XIP 使用；spi1/spi2 可保留读回但不能产生 XIP window。 | `T12-R/T53-R/RT/UB/LX` |
| 25 | 随配置变化 | `SPI_RXDS_SIG_EN` | `0x0` | HyperBus Command-Address 阶段的 `rxds` 信令使能。若 CA 阶段 `rxds=1`，地址阶段完成后发送 `2 × WAIT_CYCLES - 1` 个等待周期。 | `D` | 当前 K230 profile 未集成或未采用该字段，RAZ/WI。 | `T12-R/T53-R/RT/UB/LX` |
| 24 | 随配置变化 | `SPI_DM_EN` | `0x0` | SPI Data Mask 使能。使能后通过 `txd_dm` 屏蔽 `txd` 数据线上的数据；仅当配置参数 `SSIC_DM_EN=1` 时可用。 | `D` | 当前 K230 profile 未集成或未采用该字段，RAZ/WI。 | `T12-R/T53-R/RT/UB/LX` |
| 23:22 | R | `RSVD_SPI_CTRLR0_22_23` | `0x0` | 保留位，读取为 0。 | `D` | 保留位：读取为 0，写入忽略，不进入普通寄存器存储。 | `T12-R/T53-R/RT/UB/LX` |
| 21 | 随配置变化 | `SSIC_XIP_CONT_XFER_EN` | `0x0` | XIP continuous transfer 使能。置 1 后，SPI 保持从设备选择有效，直到 AHB 接口检测到非 XIP 传输。 | `B` | 实现写掩码、保存和读回；复杂时序或总线属性可后补。 | `T12-R/T53-R/RT/UB/LX` |
| 20 | 随配置变化 | `XIP_INST_EN` | `0x0` | XIP 指令阶段使能。置 1 后 XIP 传输包含指令阶段，操作码根据 AHB 传输类型从 `XIP_INCR_INST` 或 `XIP_WRAP_INST` 选择。 | `A/B` | spi0 决定是否从 XIP_INCR_INST/XIP_WRAP_INST 发送指令。 | `T12-R/T53-R/RT/UB/LX` |
| 19 | 随配置变化 | `XIP_DFS_HC` | `0x0` | XIP Data Frame Size 固定控制。置 1 时使用 `CTRLR0.DFS` 的固定帧长，抓取帧数由 `HSIZE/HBURST` 决定；为 0 时，帧长和帧数都由 `HSIZE/HBURST` 决定。 | `A` | 参与 XIP/DTR 数据帧组织，Linux DTR 组合值会写入该位。 | `T12-R/T53-R/RT/UB/LX` |
| 18 | 随配置变化 | `SPI_RXDS_EN` | `0x0` | Read Data Strobe 使能。置 1 后，DDR 模式使用 `rxds` 捕获读数据。 | `B` | 实现写掩码、保存和读回；复杂时序或总线属性可后补。 | `T12-R/T53-R/RT/UB/LX` |
| 17 | 随配置变化 | `INST_DDR_EN` | `0x0` | 指令阶段 DDR 使能。 | `A` | 决定指令阶段是否使用 DDR。 | `T12-R/T53-R/RT/UB/LX` |
| 16 | 随配置变化 | `SPI_DDR_EN` | `0x0` | Dual/Quad/Octal SPI 帧格式的 DDR 传输使能。 | `A` | 决定增强 SPI 数据阶段是否使用 DDR。 | `T12-R/T53-R/RT/UB/LX` |
| 15:11 | R/W | `WAIT_CYCLES` | `0x0` | Dual/Quad/Octal 模式中，控制帧发送完成到开始接收数据之间的等待周期数，以 SPI 时钟周期计。 | `A` | 直接参与指令/地址与接收数据之间的 dummy cycle 数量。 | `T12-R/T53-R/RT/UB/LX` |
| 10 | R | `RSVD_SPI_CTRLR0_10` | `0x0` | 保留位，读取为 0。 | `D` | 保留位：读取为 0，写入忽略，不进入普通寄存器存储。 | `T12-R/T53-R/RT/UB/LX` |
| 9:8 | R/W | `INST_L` | `0x2` | 指令长度：`0=INST_L0`（无指令），`1=INST_L4`，`2=INST_L8`，`3=INST_L16`。 | `A` | 指令长度编码，参与增强 SPI 与 XIP 命令生成。 | `T12-R/T53-R/RT/UB/LX` |
| 7 | 随配置变化 | `XIP_MD_BIT_EN` | `0x0` | XIP mode bits 使能。置 1 后，在地址阶段之后插入 `XIP_MODE_BITS` 中的 mode bits。 | `A/B` | spi0 置位后插入 XIP_MODE_BITS；普通 QOPI 实例无 XIP 副作用。 | `T12-R/T53-R/RT/UB/LX` |
| 6 | R | `RSVD_SPI_CTRLR0_6` | `0x0` | 保留位，读取为 0。 | `D` | 保留位：读取为 0，写入忽略，不进入普通寄存器存储。 | `T12-R/T53-R/RT/UB/LX` |
| 5:2 | R/W | `ADDR_L` | `0x0` | 地址长度。只有 FIFO 中已写入指定数量的地址位后才能开始传输：`0=0`、`1=4`、`2=8`、`3=12`、`4=16`、`5=20`、`6=24`、`7=28`、`8=32`、`9=36`、`a=40`、`b=44`、`c=48`、`d=52`、`e=56`、`f=60` 位。 | `A` | 地址长度以 4 bit 为单位，SDK 当前限制最大 32 bit。 | `T12-R/T53-R/RT/UB/LX` |
| 1:0 | R/W | `TRANS_TYPE` | `0x0` | 指令/地址传输格式：`0=TT0`，指令和地址都使用 Standard SPI；`1=TT1`，指令使用 Standard SPI、地址使用 `CTRLR0.SPI_FRF`；`2=TT2`，指令和地址都使用 `SPI_FRF`；`3=TT3`，保留。 | `A` | 决定指令/地址阶段使用 Standard 还是增强线宽。 | `T12-R/T53-R/RT/UB/LX` |
...
| 位 | 访问 | 名称 | 复位值 | 中文说明 | K230等级 | K230实现注释 | 依据 |
|---|---|---|---|---|---:|---|---|
| 31:8 | R | `RSVD_DDR_DRIVE_EDGE` | `0x0` | 保留位，读取为 0。 | `D` | 保留位：读取为 0，写入忽略，不进入普通寄存器存储。 | `T12-R/T53-R/RT/UB/LX` |
| 7:0 | R/W | `TDE` | `0x0` | TXD Drive Edge，决定发送数据的驱动边沿；最大值为 `(BAUDR / 2) - 1`。 | `B` | 实现写掩码、保存和读回；复杂时序或总线属性可后补。 | `T12-R/T53-R/RT/UB/LX` |
...
| 25 | 随配置变化 | `RXDS_SIG_EN` | `0x0` | HyperBus 命令/地址阶段 RXDS 信令使能。 | `D` | concurrent XIP 和 HyperBus 均未启用。 | `T53-R/RT` |
| 24 | 随配置变化 | `XIP_HYPERBUS_EN` | `0x0` | XIP HyperBus 帧格式使能。 | `D` | 当前 K230 SPI/OPI profile 不实现 HyperBus。 | `T53-R/RT` |
...
| 11:8 | R/W | `MAX_WS` | `0x0` | 单次事务允许的最大 wait response 次数。SPI Master 每次检查 Slave 状态并收到 wait response 时，内部计数器加 1。 | `D` | 当前 K230 profile 不采用该寄存器能力，读取为 0、写入忽略。 | `T12-R` |
| 7:3 | R | `RSVD_SPI_CTRLR1_3_7` | `0x0` | 保留位，只读。 | `D` | 保留位：读取为 0，写入忽略，不进入普通寄存器存储。 | `T12-R` |
| 2:0 | R/W | `DYN_WS` | `0x0` | 动态等待状态。当 SPI Slave 向 Master 返回 BUSY 时，在下一次状态检查前插入等待周期；等待周期数为 `DYN_WS + 1`。 | `D` | 当前 K230 profile 不采用该寄存器能力，读取为 0、写入忽略。 | `T12-R` |
...
| 0 | R | `SPITECR` | `0x0` | 反映 SPI transmit error 中断状态。读取清除 `ssi_spite_intr`；写入无效。 | `D` | 当前 K230 profile 不采用该寄存器能力，读取为 0、写入忽略。 | `T12-R` |
...
| 位 | 访问 | 名称 | 复位值 | 中文说明 | K230等级 | K230实现注释 | 依据 |
|---|---|---|---|---|---:|---|---|
| 31:16 | R | `RSVD_SPIDR` | `0x0` | 保留位，读取为 0。 | `D` | 保留位：读取为 0，写入忽略，不进入普通寄存器存储。 | `T12-R/RT/UB/LX` |
| 15:0 | R/W | `SPI_INST` | `0x0` | SPI 操作使用的指令码。 | `A` | 内部 DMA 模式下提供外部 SPI 指令，低 16 位有效。 | `T12-R/RT/UB/LX` |
...
| 位 | 访问 | 名称 | 复位值 | 中文说明 | K230等级 | K230实现注释 | 依据 |
|---|---|---|---|---|---:|---|---|
| 31:0 | R/W | `SDAR` | `0x0` | SPI 设备地址，在 DMA 传输的 SPI 接口读写操作中使用。 | `A` | 内部 DMA 模式下提供外部 SPI 设备地址。 | `T12-R/RT/UB/LX` |
...
| 位 | 访问 | 名称 | 复位值 | 中文说明 | K230等级 | K230实现注释 | 依据 |
|---|---|---|---|---|---:|---|---|
| 31:0 | R/W | `AXIAR_0_31` | `0x0` | DMA 操作 AXI 地址的低 32 位。 | `A` | 与 AXIAR1 组合为 64 位 guest physical address。 | `T12-R/RT/UB/LX` |
...
| 位 | 访问 | 名称 | 复位值 | 中文说明 | K230等级 | K230实现注释 | 依据 |
|---|---|---|---|---|---:|---|---|
| 31:y | R | `RSVD_AXIAR1` | `0x0` | 保留位，读取为 0。 | `D` | 保留位：读取为 0，写入忽略，不进入普通寄存器存储。 | `T12-R/RT/UB/LX` |
| x:0 | R/W | `AXIAR_32_63` | `0x0` | DMA 源地址的高位部分。 | `A` | 当前 K230 SDK 实际写入 guest physical address 高 32 位。 | `T12-R/RT/UB/LX` |
...
| 位 | 访问 | 名称 | 复位值 | 中文说明 | K230等级 | K230实现注释 | 依据 |
|---|---|---|---|---|---:|---|---|
| 31:1 | R | `RSVD_AXIECR` | `0x0` | 保留位。 | `D` | 保留位：读取为 0，写入忽略，不进入普通寄存器存储。 | `T12-R/RT/UB/LX` |
| 0 | RC | `AXIECR` | `0x0` | 反映 AXI error 中断状态。读取清除 `ssi_axie_intr`；写入无效。 | `C` | 读取返回并清除 AXI error 锁存；写入忽略。 | `T12-R/RT/UB/LX` |
...
| 位 | 访问 | 名称 | 复位值 | 中文说明 | K230等级 | K230实现注释 | 依据 |
|---|---|---|---|---|---:|---|---|
| 31:1 | R | `RSVD_DONECR` | `0x0` | 保留位。 | `D` | 保留位：读取为 0，写入忽略，不进入普通寄存器存储。 | `T12-R/RT/UB/LX` |
| 0 | RC | `DONECR` | `0x0` | 反映 transfer done 中断状态。读取清除 `ssi_done_intr`；写入无效。 | `C` | 读取返回并清除内部 DMA DONE 锁存；写入忽略。 | `T12-R/RT/UB/LX` |
...
| 13 | 随配置变化 | `XIPWR_RXDS_SIG_EN` | `0x0` | HyperBus 命令/地址阶段 RXDS 信令使能。 | `D` | XIP write 和 HyperBus 均未启用。 | `T53-R/RT` |
| 12 | 随配置变化 | `XIPWR_HYPERBUS_EN` | `0x0` | XIP write HyperBus 帧格式使能。 | `D` | 当前 K230 SPI/OPI profile 不实现 HyperBus。 | `T53-R/RT` |
...
| 内部 AXI DMA | 支持 Flash 读写 | 支持多线读写 | SDK 选择 `SSIC_HAS_DMA = 2`，必须实现 |
...
- `k230_sdk/src/little/linux/drivers/spi/spi-dw-core-k230.c`
...
| `UB` | U-Boot `designware_spi.c` | DWC SSI 字段探测、内部 DMA、DTR 和 `SSI_CTRL` 的实际布局 |
| `LX` | Linux `spi-dw-core-k230.c`、`spi-dw-k230.h` | K230 SPI/QSPI、clock stretch、DTR 和中断处理路径 |
...
证据使用顺序不是简单地“SDK 一定覆盖 TRM”。物理地址和实例归属优先看 `MAP/DTS`；通用行为优先看 `T12/T53`；字段是否集成优先看 SDK 结构体、条件编译和实际访问；仍冲突时标记待 `HW` 验证。
...
| `B` | 字段存在，但当前 SDK 路径不依赖其完整效果 | 实现写掩码、保存和读回；复杂行为可独立后补 |
| `C` | 固定值、动态只读状态或 RC 清除口 | 读取时计算或返回固定值，按定义处理读取清除，写入忽略 |
| `D` | 当前 K230 profile 未集成或明确不支持 | RAZ/WI，不能因 Guest 写入而启用该能力 |
...
| DMA 形态 | 内部 AXI DMA | 内部 AXI DMA | `RT/UB` 中 `SSIC_HAS_DMA = 2` |
...
|---|---:|---|---|---|
| `CTRLR0.SSI_IS_MST` | `D` | 三个实例固定为 Master，不提供 Guest 可切换的 Master/Slave 综合位 | 读 0、写忽略；设备内部固定执行 Master 行为 | `T12-R`、`RT/UB/LX` 均按 Master 使用 |
| `CTRLR0.SPI_DWS_EN` | `D` | 动态等待状态未集成 | RAZ/WI；不得使 `0x118` 变成有效寄存器 | `T12-R`、`RT` 将 `0x118` 设为保留 |
| `CTRLR0.SPI_HYPERBUS_EN` | `D` | 当前 SDK 使用 SPI/QSPI/OPI，不使用 HyperBus | RAZ/WI | `T12-R`、`RT/UB/LX` 无 HyperBus 路径 |
| `CTRLR0.SPI_FRF` | `A` | Standard/Dual/Quad；仅 `spi0` 允许 Octal | 参与指令、地址和数据线宽选择；`spi1/spi2` 的 Octal 写值不能获得八线能力 | `T12-F/T53-F`、`RT/UB/LX` |
| `CTRLR0.SRL` | `B` | K230 Linux 通用 SPI mode 可以写 loopback 位，但当前启动路径不依赖 | 先实现位保存和读回；实现 `SPI_LOOP` 时再连接内部回环数据路径 | `T12-R`、`LX` 中 `SPI_LOOP` |
| `CTRLR0.SCPOL` | `A` | SPI mode 的时钟极性 | 参与串行模式配置；高层 SSI 总线无法表达边沿时至少必须正确读回 | `T12-F/T12-R`、`RT/UB/LX` |
| `CTRLR0.SCPH` | `A` | SPI mode 的时钟相位 | 同 `SCPOL` | `T12-F/T12-R`、`RT/UB/LX` |
| `CTRLR0.FRF` | `A` | K230 路径只采用 Motorola SPI，不能由该字段启用 Microwire/SSP | 字段按掩码读写，但只有 `FRF=0` 进入支持的数据路径；其他值不得误用 SPI 行为 | `T12-R`、`RT/UB/LX` |
| `MWCR.MHS` | `D` | Microwire 未作为 K230 SPI profile 能力使用 | 整个 `MWCR` 按 RAZ/WI；`MDD/MWMOD` 同样不实现 | `T12-R`、`RT/UB/LX` 无 Microwire 路径 |
| `TXFTLR.TXFTHR` | `A` | K230 选择 11 位字段；SDK 用它控制多线和 IDMA 起步水位 | 保存 `bits[26:16]`，按“TX FIFO 项数大于该值”决定是否开始串行发送 | `T12-F/T12-R`、`RT` 实际写入 |
| `RX_SAMPLE_DELAY.SE` | `A` | 字段存在；SDK 与实例配置共同决定采样边沿 | 保存 bit 16；与 `RSD[7:0]` 一起构成寄存器读回和 profile 配置 | `T12-F/T12-R`、`RT/UB/LX` |

#### `IMR` 中的条件中断字段

| 字段 | 等级 | K230 裁决 | 实现要求 | 依据 |
|---|---:|---|---|---|
| `DONEM` bit 11 | `A` | 内部 DMA完成中断存在 | 与 `DONECR` 和第 8 路 IRQ 联动 | `T12-R`、`RT/UB/LX` |
| `SPITEM` bit 10 | `D` | 动态等待状态和 `SPITECR` 未集成 | 读 0、写忽略，不产生 SPITE 状态 | `RT` 保留 `0x118/0x11c` |
| `AXIEM` bit 8 | `A` | 内部 DMA AXI error 中断存在 | 与 `AXIECR` 和第 9 路 IRQ 联动 | `T12-R`、`RT/UB/LX` |
...
| 字段 | 等级 | K230 裁决 | 实现要求 | 依据 |
|---|---:|---|---|---|
| `AID[18:15]` | `B` | SDK 明确按 4 位布局 | 保存和读回；QEMU 不必模拟 AXI ID 仲裁效果 | `UB/LX` 的 `spi_dmacr_t` |
| `APROT[14:12]` | `B` | 字段存在，SDK通常写 0 | 保存和读回；不必改变 QEMU address space 权限模型 | `T12-R`、`UB/LX` |
| `ACACHE[11:8]` | `B` | 字段存在，SDK通常写 0 | 保存和读回；不必模拟 AXI cache 属性 | `T12-R`、`UB/LX` |
| `AINC` bit 6 | `A` | K230 SDK 统一采用 `1=Increment` | IDMA 每个 beat 后按 `ATW` 递增 guest memory 地址 | `T12-R` 位域表、`RT/UB/LX` 实际置位 |
| `ATW[4:3]` | `A` | SDK 使用 `3=8 bytes` | 决定每个 AXI beat 的字节数；不得误作 SPI 帧宽 | `T12-F/T12-R`、`RT/UB/LX` |
| `IDMAE` bit 2 | `A` | `SSIC_HAS_DMA=2` 的主数据路径 | 置位后使用 `SPIDR/SPIAR/AXIAR` 执行内部 DMA | `T12-F`、`RT/UB/LX` |
| `TDMAE` bit 1 | `D` | 属于外部 DMA请求接口，K230 当前选择内部 DMA变体 | RAZ/WI | `RT/UB` 中 `SSIC_HAS_DMA=2` 条件布局 |
| `RDMAE` bit 0 | `D` | 同上 | RAZ/WI | `RT/UB` 中 `SSIC_HAS_DMA=2` 条件布局 |
...
其中 `AID/APROT/ACACHE` 可以只保存；`AINC/ATW/IDMAE` 必须实际参与 IDMA 行为。

#### `SPI_CTRLR0` 中的条件字段

| 字段 | 等级 | FMC/OPI `spi0` | SPI/QOPI `spi1/spi2` | 依据 |
|---|---:|---|---|---|
| `CLK_STRETCH_EN` bit 30 | `A` | 有效 | 有效；Linux 增强 SPI 路径主动置位 | `T12-R/T53-R`、`LX` |
...
| `SPI_RXDS_SIG_EN` bit 25 | `D` | HyperBus 专用，当前不实现 | RAZ/WI | `T12-R/T53-R`、SDK 无 HyperBus |
| `SPI_DM_EN` bit 24 | `D` | 当前 SDK 未选择 SPI data mask 能力 | RAZ/WI | `T12-R`、SDK 无 `SSIC_DM_EN` 路径 |
...
| `XIP_DFS_HC` bit 19 | `A` | 参与 XIP/DTR 数据帧组织 | Linux DTR 组合值会写到该位，必须保留并参与增强事务 | `T12-R/T53-R`、`LX` |
| `SPI_RXDS_EN` bit 18 | `B` | RXDS 捕获选择；先保存和读回，精确采样时序随 RXDS 模型补齐 | 保存和读回但不创建 RXDS 线路 | `T12-F/T53-F`、`UB` 中相关配置 |
| `INST_DDR_EN` bit 17 | `A` | 指令阶段 DDR | 增强 SPI DTR 指令阶段 | `T12-R/T53-R`、`UB/LX` |
| `SPI_DDR_EN` bit 16 | `A` | 数据/地址 DDR | 增强 SPI DTR 数据阶段 | `T12-R/T53-R`、`UB/LX` |
...
```c
#define K230_SSI_SPI_CTRLR0_WRITABLE_MASK 0x6c3ffbbfU
```

该掩码允许 K230 证据支持的 SPI、DTR 和基础 XIP 字段；bit 25、24、23、22、10、6 以及 bit 31、28 等保留位仍为 RAZ/WI。profile 决定字段是否产生 XIP 副作用，而不是靠修改寄存器掩码动态创建 XIP window。
...
|---|---|---|---|
| `SER` 宽度 | 1 位，只有 bit 0 | `writable_mask=0x1`；bit 1–31 RAZ/WI | `DTS num-cs=1`、`RT` |
...
| `0x050/0x054` | `AXIAWLEN/AXIARLEN` | 不实现为 `DMATDLR/DMARDLR` | `SSIC_HAS_DMA=2`、`RT/UB` |
| `AWLEN/ARLEN` | bit 15:8，共 8 位 | `writable_mask=0x0000ff00` | `UB/LX` 结构体 |
| `DMACR.AID` | bit 18:15，共 4 位 | 只保存 4 位 | `UB/LX` 结构体 |
...
| `0x120–0x134` | 内部 DMA寄存器存在 | 必须实现 | `SSIC_HAS_DMA=2`、`RT/UB/LX` |
...
|---:|---|---:|---|---|
| `0x000` | `CTRLR0` | `A/B` | 可写 `0x00c06fdf`；其中去掉 `SRL` 的 `0x00c04fdf` 必须进入实际事务行为 | `T12-R`、`RT/UB/LX` |
| `0x004` | `CTRLR1` | `A` | `NDF[15:0]`，可写 `0x0000ffff` | `T12-R`、`RT/UB/LX` |
| `0x008` | `SSIENR` | `A` | 仅 bit 0；清零停止传输并清空 FIFO | `T12-F/T12-R`、SDK 写序列 |
| `0x00c` | `MWCR` | `D` | RAZ/WI | SDK 无 Microwire |
| `0x010` | `SER` | `A` | 仅 bit 0；选择唯一硬件片选 | `DTS`、`RT` |
...
| `0x04c` | `DMACR` | `A/B` | 可写 `0x0007ff5c` | `T12-F/T12-R`、`RT/UB/LX` |
| `0x050` | `AXIAWLEN` | `A` | 可写 `0x0000ff00` | `SSIC_HAS_DMA=2`、`UB/LX` |
| `0x054` | `AXIARLEN` | `A` | 可写 `0x0000ff00` | `SSIC_HAS_DMA=2`、`UB/LX` |
...
| `0x0f0` | `RX_SAMPLE_DELAY` | `A` | 可写 `0x000100ff` | `T12-R`、`RT/UB/LX` |
| `0x0f4` | `SPI_CTRLR0` | `A/B` | 可写 `0x6c3ffbbf`；复位值按 profile 区分 | `T12-R/T53-R`、SDK |
| `0x0f8` | `DDR_DRIVE_EDGE` | `B` | 可写 `0xff`；保存读回，精确边沿时序可后补 | `T12-R/T53-R`、`UB/LX` |
...
| 偏移 | 寄存器 | 等级 | 实现要求 | 依据 |
|---:|---|---:|---|---|
| `0x120` | `SPIDR` | `A` | 保存低 16 位 SPI 指令并参与 IDMA事务 | `T12-R`、`RT/UB/LX` |
| `0x124` | `SPIAR` | `A` | 保存 32 位外部 SPI 地址 | `T12-R`、`RT/UB/LX` |
| `0x128` | `AXIAR0` | `A` | guest memory 地址低 32 位 | `T12-R`、`RT/UB/LX` |
| `0x12c` | `AXIAR1` | `A` | guest memory 地址高 32 位 | `T12-R`、`RT` |
| `0x130` | `AXIECR` | `C` | 读取返回并清除 AXI error 锁存 | `T12-R`、`RT/UB/LX` |
| `0x134` | `DONECR` | `C` | 读取返回并清除 DONE 锁存 | `T12-R`、`RT/UB/LX` |
...
该布局来自 `UB` 的位域结构体和 `SSI_CTRL = 0x91585000 + 0x68` 绝对地址；它是对 TRM 错位表格的实现裁决，不修改前面的 TRM 原始翻译。
...
| `SSI_CTRL` | `0x00004000` | 独立 HI_SYS_CONFIG 寄存器 | `T12-R/T53-R`、`UB` |
...
还要区分“写值不合法”和“字段不存在”：例如 `spi1/spi2` 的 `SPI_FRF=Octal` 是现有字段选择了实例无法提供的线宽，不能因此创建八线能力；HyperBus 和动态等待字段则属于当前 profile 不存在，直接 RAZ/WI。
...
该顺序只表示依赖关系，不表示后面的能力可以永久省略。严格服务 K230 SPI/QSPI/OPI SDK 的完成条件仍包括内部 DMA、九路 IRQ、FMC profile 和 `SSI_CTRL`。
...
- `qemu-camp-2026-k230/hw/ssi/k230_dw_ssi.c`
- `qemu-camp-2026-k230/include/hw/ssi/k230_dw_ssi.h`
- `qemu-camp-2026-k230/hw/riscv/k230.c`
- `qemu-camp-2026-k230/tests/qtest/k230-dw-ssi-test.c`
- `qemu-camp-2026-k230/docs/devel/k230-spi-qspi.rst`
- `k230_sdk/src/big/rt-smart/kernel/bsp/maix3/board/interdrv/spi/drv_spi.c`
- `k230_sdk/src/little/uboot/drivers/spi/designware_spi.c`
- `k230_sdk/src/little/linux/drivers/spi/spi-dw-core-k230.c`

> [!NOTE]
> **证据边界：** 以下状态是 2026-07-13 对当前 `spi` 分支的实现快照。表中的“已完成”只表示当前 QEMU 代码已经存在对应路径，不表示它已经完整符合 TRM、SDK 或真实硬件行为。

### 当前实现结论

当前模型已经不是空设备：三个 SSI 实例、SPI NOR 后端、字节级 PIO、简化中断状态和只读 XIP window 均已建立。非 DMA MMIO 白名单、写掩码和保留地址策略已经加入，现有 8 个 qtest 全部通过。
...
| SPI NOR | 基础可用 | SPI/OPI 实例 CS0 连接 `w25q256`，支持 `-drive if=mtd` |
| 标准 SPI PIO | 原型可用 | 可通过 `DRx` 执行字节级 `ssi_transfer()` |
| FIFO | 部分实现 | RX FIFO 有效；TX FIFO 尚未真正参与发送过程 |
| 中断 | 部分实现 | 只计算 TXE/RXF 条件，外部 IRQ 尚未接入 PLIC |
| 增强 QSPI/OPI | 未形成实际事务 | 配置寄存器可以回读，但字段没有驱动指令、地址、dummy 和数据阶段 |
| XIP window | 原型可用 | `0xC0000000–0xC7FFFFFF` 可读取同一 SPI NOR 后端 |
| 内部 AXI DMA | 当前不实现 | 本次 MMIO patch 将相关地址作为保留；SDK 使用 DMA 的事实保留为后续兼容性证据 |
| QSPI 冷启动 | 未验证 | 仍需控制器能力补全后与 BootROM、DDRC 和镜像链路联合验收 |

### 当前 qtest 覆盖

2026-07-13 重新编译并执行当前工作区的 `k230-dw-ssi-test`，结果为 `8/8 PASS`。该结果证明当前 MMIO 掩码、保留地址和已有基础传输路径内部一致，但不能证明寄存器副作用、完整 QSPI 事务或启动链已经实现。

| 测试 | 当前验证内容 | 尚未证明的内容 |
|---|---|---|
| `reset_values` | 三个实例可访问，早期基础复位值可读 | FMC/OPI 与 SPI/QOPI 的分 profile 复位值、完整写掩码 |
| `register_readback` | 基础配置寄存器和 XIP opcode 可以保存和回读 | 配置寄存器在控制器使能期间的写入约束 |
| `register_write_masks` | RW 寄存器只更新 writable bits，并保留只读复位位 | 控制器使能期间是否允许修改各配置寄存器 |
| `read_only_and_reserved_registers` | RO 写入无效；DMA、concurrent-XIP 和 dynamic-wait 地址读零写忽略 | RC 寄存器的读取清除副作用 |
| `jedec_id` | CS0、字节级 PIO 和 `w25q256` 连接有效 | 普通读取、页编程、擦除和多帧宽度 |
| `disabled_dr_write_does_not_stale_fifo` | 控制器禁用时，DR 写入不会留下 RX 数据 | `SSIENR=0` 主动清空已有 TX/RX FIFO |
| `internal_interrupt_status` | RX FIFO 水位可反映到简化的 `RISR/ISR` | 错误事件锁存、读清寄存器和 PLIC 外部中断 |
| `xip_window_read` | 1/2/4/8 字节读取及 `0x03/0x13` 地址模式 | mode bits、片选选择、`SPI_CTRLR0` 驱动的地址/dummy 和连续传输语义 |

现有测试证明当前简化实现内部一致，但不能单独作为“符合 TRM”或“能够启动 U-Boot/Linux”的证据。

### 当前 MMIO 访问策略

本轮先实现寄存器访问层，不补传输、中断清除、DMA 或 XIP 控制副作用。普通 RW 寄存器统一使用以下更新方式：
...
该写法与简单的 `guest_value & writable_mask` 不同：前者会保留只读配置位和复位位。例如 `SPI_CTRLR0.XIP_MBL` 不在 writable mask 中，但其 profile 复位值仍应在其他字段写入后继续可见。

读取普通存储寄存器时暂不再次应用 read mask，而是直接返回 `regs[]`。保留位能够维持为 0，依赖两个前提：

1. reset 只写入已确认的复位值；
2. guest 写入必须经过 writable mask。

| 寄存器 | 当前 writable mask | 证据和说明 |
|---|---:|---|
| `CTRLR0` | `0x00cf7f1f` | 保留 `DFS`、`SCPH/SCPOL`、`TMOD`、`SLV_OE`、`SRL`、`SSTE`、`CFS`、`SPI_FRF`；U-Boot 会写全 1 后回读以探测 `DFS32` 和 enhanced SPI 能力 |
...
| `TXFTLR` | `0x00ff00ff` | `TFT[7:0]` 与 `TXFTHR[23:16]`；RT-Smart 会同时写两个字段 |
| `RXFTLR` | `0x000000ff` | K230 SDK 的 256 深度配置需要 8 位阈值 |
| `IMR` | `0x000000bf` | 当前非 DMA 范围保留 TXE/TXO/RXU/RXO/RXF/MST/TXU；排除 XIP overflow、AXI error、dynamic-wait error 和 DONE |
| `RX_SAMPLE_DELAY` | `0x000100ff` | SDK 实际写入 `rdse << 16 | rdsd` |
| `SPI_CTRLR0` | `0x4007fbbf` | 与 K230 U-Boot 使用的 `TRANS_TYPE/ADDR_L/XIP_MD_BIT_EN/INST_L/WAIT_CYCLES/DDR/RXDS/CLK_STRETCH` 对齐；其他可选扩展位不开放 |
| `DDR_DRIVE_EDGE` | `0x000000ff` | `TDE[7:0]` |
| `XIP_MODE_BITS` | `0x0000ffff` | 仅 `has-xip=true` 的 SPI/OPI 实例可读写 |
| `XIP_INCR_INST` | `0x0000ffff` | 仅 XIP 实例可读写 |
| `XIP_WRAP_INST` | `0x0000ffff` | 仅 XIP 实例可读写 |

寄存器读取和写入按下列类型处理：

| 类型 | 当前读行为 | 当前写行为 | 当前是否实现副作用 |
|---|---|---|---|
| 普通 RW | 返回保存值，不额外 mask | 保留只读位，只更新 writable mask | 否 |
| 动态 RO | 由 FIFO、状态或中断辅助函数计算 | 忽略 | 保留已有简化行为 |
| 固定 RO | 返回 `IDR` 或 `SSIC_VERSION_ID` | 忽略 | 不需要 |
| RC 寄存器 | 当前返回 0 或已有占位状态 | 忽略 | 尚未实现读取清除 |
| `DR0–DR35` | 从 RX FIFO 读取 | 保留现有字节传输路径 | 保留现有原型 |
| XIP-only RW | 非 XIP 实例返回 0 | 非 XIP 实例忽略 | 仅保存字段，XIP 数据读取行为保持原状 |
| DMA/可选扩展/保留地址 | 返回 0 | 忽略 | 不实现 |
...
K230 Linux 驱动则确认 DWC SSI 布局下使用 `CTRLR0.DFS[4:0]`、`TMOD[11:10]`、`SRL[13]` 和 `SPI_FRF[23:22]`，这也是当前 `CTRLR0` mask 与旧版通用 DesignWare 位图不同的原因。
...
总体上游拆分继续按能力记录在下表；本轮只处理第 1 个 patch 中的非 DMA MMIO 访问层。完整平台启动测试在 BootROM、DDRC 和镜像系列具备后单独加入。

| 顺序 | 建议 Patch | 目标 | 当前状态 | 当前主要缺口 |
|---:|---|---|---|---|
| 1 | `hw/ssi: Add a K230 DesignWare SSI controller model` | 建立 QOM、MMIO、寄存器、复位和迁移框架 | MMIO 基础层已完成 | 后续仍需确认 FMC/OPI 与 SPI/QOPI 的差异化 profile，以及配置寄存器的禁用期写约束 |
| 2 | `hw/riscv/k230: Instantiate the K230 SSI controllers` | 接入三个实例、地址、能力和 PLIC IRQ | 部分完成 | 三个实例与 MMIO 已接入，但 SSI IRQ 没有连接到 PLIC |
| 3 | `hw/ssi: Implement K230 SSI FIFO and PIO transfers` | 实现 DR、TX/RX FIFO、状态、帧宽和传输模式 | 原型可用 | TX FIFO 未参与发送；固定发送低 8 位；`DFS/TMOD` 未生效；`BUSY` 未实现 |
| 4 | `hw/ssi: Implement K230 SSI interrupt handling` | 实现水位、错误、读清和外部 IRQ | 少量实现 | 仅有 TXE/RXF；没有错误锁存和 RC 行为；当前单 IRQ 接口与 SDK 中断拓扑不符 |
| 5 | `hw/riscv/k230: Attach SPI NOR flash to K230 SSI` | 接入 SPI NOR 和 MTD 镜像 | 基础完成 | 只有 JEDEC ID 测试；尚未验证真实 U-Boot 驱动使用的完整命令序列 |
| 6 | `hw/ssi: Implement K230 enhanced SPI/QSPI transfers` | 实现 Dual/Quad/Octal 软件可见事务 | 未实现 | `SPI_FRF`、`TRANS_TYPE`、`ADDR_L`、`INST_L`、`WAIT_CYCLES` 和 `NDF` 未参与传输 |
| 7 | `hw/ssi: Implement K230 internal AXI DMA` | 实现 `SPIDR/SPIAR/AXIAR`、DONE/AXIE 和 guest memory 搬运 | 不在当前范围 | 当前统一按保留地址处理；只有启动固件实际依赖该路径时才重新立项 |
| 8 | `hw/ssi: Add the K230 SPI flash XIP window` | 实现 Flash 映射读取窗口 | 原型可用 | `XIP_MODE_BITS` 未生效；dummy 和地址长度主要按 opcode 硬编码 |
| 9 | `hw/misc: Model K230 HI_SYS SSI_CTRL` | 在 `0x91585068` 提供 XIP、sleep、mode 和 RXDS 包装控制 | 未实现 | 当前 HI_SYS_CONFIG 仍是占位设备 |
| 10 | `docs: Document K230 SPI/QSPI support` | 记录地址、profile、启动参数、支持范围和限制 | 已有草稿 | 上游文档需要使用英文，并在实现稳定后重新校准功能声明 |
| 11 | `tests/functional: Add a K230 QSPI boot test` | 验证 BootROM → QSPI → U-Boot/Linux | 等待平台集成 | 依赖其他 K230 启动组件和完整 SDK 访问路径 |
...
因此不建议继续在这三个提交之后堆叠修正 patch。更合适的方式是在确认实现语义后，按前述 8 个能力边界重新组织提交历史，保证每个 patch 独立编译、功能边界清晰，并带有与该能力对应的测试。

> [!NOTE]
> **当前范围决策：** 本轮只建立非 DMA MMIO 访问层，不实现内部 AXI DMA。`DMACR`、`AXIAWLEN/AXIARLEN`、`SPIDR`、`SPIAR`、`AXIAR0/1`、`AXIECR` 和 `DONECR` 当前读零、写入忽略。SDK 使用这些寄存器的事实继续保留；如果后续 BootROM 或 U-Boot 实际启动路径依赖内部 DMA，再以独立 patch 重新设计 guest memory 搬运与 DONE/AXIE 中断。
