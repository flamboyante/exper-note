...
|---|---|---|---|---:|---|
| `0x000` | `CTRLR0` | 控制寄存器 0 | 可写掩码 `0x00cf7f1f`；`DFS/TMOD/SPI_FRF/SCPOL/SCPH` 进入事务行为，`CFS/SLV_OE/SRL/SSTE` 保存读回 | `A/B` | `T12-R`、`RT/UB-ACT/LX-ACT` |
| `0x004` | `CTRLR1` | 控制寄存器 1 | `NDF[15:0]` 可写，决定 `NDF+1` 帧 | `A` | `T12-R`、`RT/UB-ACT/LX-ACT` |
| `0x008` | `SSIENR` | SSI 使能寄存器 | 仅 bit 0；清零停止传输并清空 TX/RX FIFO | `A` | `T12-F/T12-R` |
| `0x00c` | `MWCR` | Microwire 控制寄存器 | `MWMOD/MDD` 按 TRM R/W 保存读回，当前 Motorola SPI 路径不产生 Microwire 副作用；`MHS` 待集成证据 | `B/D` | `T12-R`、`RT/UB-ACT/LX-ACT/HW` |
| `0x010` | `SER` | 从设备使能寄存器 | 按实例 `num-cs` 生成有效掩码；Linux DTS 为 1，U-Boot DTS 的 spi1/spi2 为 5 | `A` | `DTS`、`RT/UB-ACT/LX-ACT` |
...
| `0x04c` | `DMACR` | DMA 控制寄存器 | 内部 DMA profile；可写 `0x0007ff5c`，`IDMAE/AINC/ATW` 参与搬运 | `A/B` | `T12-F/T12-R`、`RT/UB-ACT/LX-ACT` |
| `0x050` | `DMATDLR` | 外部 DMA 发送水位寄存器 | 与 K230 的 `SSIC_HAS_DMA=2` 内部 DMA 布局互斥，当前地址按 `AXIAWLEN` 实现 | `D` | `RT/UB-ACT` |
| `0x050` | `AXIAWLEN` | 内部 DMA目的端 burst 长度 | `AWLEN[15:8]` 可写，mask `0xff00` | `A` | `T12-R`、`RT/UB-ACT/LX-ACT` |
| `0x054` | `DMARDLR` | 外部 DMA 接收水位寄存器 | 与 K230 的 `SSIC_HAS_DMA=2` 内部 DMA 布局互斥，当前地址按 `AXIARLEN` 实现 | `D` | `RT/UB-ACT` |
| `0x054` | `AXIARLEN` | 内部 DMA源端 burst 长度 | `ARLEN[15:8]` 可写，mask `0xff00` | `A` | `T12-R`、`RT/UB-ACT/LX-ACT` |
...
| `0x068` | `SSI_CTRL` | TRM 混入的 SoC 控制寄存器 | 不属于 SPI `base+0x068`；实际地址 `0x91585068`，SPI 此偏移是 `DR2` | `E` | `MAP`、`UB-ACT`、`RT` |
| `0x0f0` | `RX_SAMPLE_DELAY` | RX 采样延迟寄存器 | `SE[16]`、`RSD[7:0]` 可写，mask `0x000100ff` | `A` | `T12-R`、`RT/UB-ACT/LX-ACT` |
| `0x0f4` | `SPI_CTRLR0` | SPI 控制寄存器 0 | 实现位掩码 `0x6f3ffbbf`；`spi0` 复位 `0x28000200`，`spi1/2` 复位 `0x04000200` | `A/B` | `T12-R/T53-R`、`RT/UB-ACT/LX-ACT/HW` |
| `0x0f8` | `DDR_DRIVE_EDGE` | 发送驱动边沿寄存器 | 低 8 位可写；先保证掩码与读回 | `B` | `T12-R/T53-R`、`UB-ACT/LX-ACT` |
...
| `0x120` | `SPIDR` | 内部 DMA SPI 指令寄存器 | 低 16 位可写并参与 IDMA 事务 | `A` | `T12-R/T53-R`、`RT/UB-ACT/LX-ACT` |
| `0x124` | `SPIAR` | 内部 DMA SPI 地址寄存器 | 32 位可写并参与 IDMA 事务 | `A` | `T12-R/T53-R`、`RT/UB-ACT/LX-ACT` |
| `0x128` | `AXIAR0` | 内部 DMA 内存地址低位 | 与 `AXIAR1` 组合成 64 位 guest physical address | `A` | `T12-R`、`RT/UB-ACT/LX-ACT` |
| `0x12c` | `AXIAR1` | 内部 DMA 内存地址高位 | 当前按高 32 位实现 | `A` | `T12-R`、`RT` |
| `0x130` | `AXIECR` | AXI Master 错误中断清除 | 读取清除 AXIE，写入忽略 | `C` | `T12-R`、`RT/UB-ACT/LX-ACT` |
| `0x134` | `DONECR` | 传输完成中断清除 | 读取清除 DONE，写入忽略 | `C` | `T12-R`、`RT/UB-ACT/LX-ACT` |
...
> - **`bits[25] SPI_DWS_EN`、`bits[24] SPI_HYPERBUS_EN`：** dynamic wait 有 RT 条件布局的直接排除证据，按 RAZ/WI；HyperBus 只有“软件未使用”的旁证，不足以证明硅上 RAZ/WI，因此保存/读回能力与真实值标记待 `HW`，当前模型不产生 HyperBus 事务。
...
> - **写入实现：** 仅在 `SSIENR=0` 时更新可写字段；推荐 `writable_mask=0x00cf7f1f`，保留 `DFS/SCPH/SCPOL/TMOD/SLV_OE/SRL/SSTE/CFS/SPI_FRF`，排除固定 `FRF`、dynamic wait 和保留位。`SPI_FRF=3` 在 spi1/spi2 上不得创建八线能力。
...
| `CTRLR0.SPI_HYPERBUS_EN` | `B/HW` | TRM 列出条件字段，当前软件未使用 HyperBus，但这不足以证明字段不存在 | 保存/读回能力与精确读值待 `HW`；当前模型不产生 HyperBus 副作用 | `T12-R`、`RT/UB-ACT/LX-ACT/HW` |
...
| `0x120–0x134` | 内部 DMA寄存器存在 | 必须实现 | `SSIC_HAS_DMA=2`、`RT/UB-ACT/LX-ACT` |
...
|---:|---|---:|---|---|
| `0x000` | `CTRLR0` | `A/B` | 可写 `0x00cf7f1f`；`DFS/TMOD/SPI_FRF/SCPOL/SCPH` 进入事务，`CFS/SLV_OE/SRL/SSTE` 保存读回，`FRF` 固定 Motorola SPI | `T12-R`、`RT/UB-ACT/LX-ACT` |
...
当前模型已经不是空设备：三个 SSI 实例、SPI NOR 后端、字节级 PIO、简化中断状态和只读 XIP window 均已建立。但多数普通控制寄存器仍直接保存完整 32 位，尚未实现本章裁决的统一 `implemented_mask/writable_mask`；当前 6 个 qtest 全部通过，只覆盖现有原型行为。
...
| 复位与固定只读值 | 部分实现 | `CTRLR0/SR/SSIC_VERSION_ID` 基础值存在；`IDR` 当前返回 0，`BAUDR` 当前复位为 2，`SPI_CTRLR0` 尚未按 spi0/spi1/2 profile 区分复位值 |
| 增强 QSPI/OPI | 未形成实际事务 | 配置寄存器可以回读，但字段没有驱动指令、地址、dummy 和数据阶段 |
| XIP window | 原型可用 | `0xC0000000–0xC7FFFFFF` 可读取同一 SPI NOR 后端 |
| 内部 AXI DMA | 当前不实现 | `DMACR/0x050/0x054` 仍是普通保存值，`0x120–0x134` 超出当前寄存器数组范围；没有实际 guest memory 搬运、DONE 或 AXIE 行为 |
| QSPI 冷启动 | 未验证 | 仍需控制器能力补全后与 BootROM、DDRC 和镜像链路联合验收 |

### 当前 qtest 覆盖

2026-07-14 使用当前构建目录重新执行 `k230-dw-ssi-test`，结果为 `6/6 PASS`。该结果证明现有基础寄存器读回、字节级 JEDEC ID、简化水位状态和只读 XIP 原型内部一致，但不能证明本章目标掩码、RC、副作用、完整 QSPI 事务或启动链已经实现。

| 测试 | 当前验证内容 | 尚未证明的内容 |
|---|---|---|
| `reset_values` | 三个实例可访问，早期基础复位值可读 | FMC/OPI 与 SPI/QOPI 的分 profile 复位值、完整写掩码 |
| `register_readback` | 基础配置寄存器和 XIP opcode 的当前保存/截断行为 | TRM/K230 的完整 `implemented_mask/writable_mask`、保留位和禁用期写入约束 |
...
现有 6 项测试证明当前简化实现内部一致，但不能单独作为“符合 TRM”或“能够启动 U-Boot/Linux”的证据。
...
| 内部 DMA RW | 返回保存值或动态 DMA 状态 | 按掩码保存 `DMACR/SPIDR/SPIAR/AXIAR` 等字段 | `IDMAE` 启动后执行 SPI 与 guest memory 搬运，并产生 DONE/AXIE |
...
总体上游拆分继续按能力记录在下表。当前代码只达到第 1 个 patch 的寄存器/迁移原型，仍未完成本章要求的访问掩码和条件地址裁决；完整平台启动测试在 BootROM、DDRC 和镜像系列具备后单独加入。

| 顺序 | 建议 Patch | 目标 | 当前状态 | 当前主要缺口 |
|---:|---|---|---|---|
| 1 | `hw/ssi: Add a K230 DesignWare SSI controller model` | 建立 QOM、MMIO、寄存器、复位和迁移框架 | 原型已建立 | 补齐本章逐寄存器 `implemented_mask/writable_mask`、分 profile 复位值、条件地址 RAZ/WI 和禁用期写约束 |
